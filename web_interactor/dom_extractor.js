...
//    jira_type_issue,calendar_dom_content,calendar_event_dom_content,calendar_event_box_dom_content,test,hubspot_dom_content,hubspot_create_company_dom_content}=require('./dom')
...
var cnt=0
function recur(root,ans,index,goal){
    var counter=0;
    var map={}
    ...
    const new_root_idx_arr=index.search(goal)
    ...
    for(var j=0;j<new_root_idx_arr.length;j++){
        ...
        if(root.childNodes[map[new_root_idx_arr[j]]]  && root.childNodes[map[new_root_idx_arr[j]]].rawTagName!='iframe' && root.childNodes[map[new_root_idx_arr[j]]].rawTagName!='script' && root.childNodes[map[new_root_idx_arr[j]]].rawTagName!='picture' && root.childNodes[map[new_root_idx_arr[j]]].rawTagName!='style' && root.childNodes[map[new_root_idx_arr[j]]].rawTagName!='code' && root.childNodes[map[new_root_idx_arr[j]]].rawTagName!='img' && root.childNodes[map[new_root_idx_arr[j]]].rawTagName!='use'){
            ...
            if(index_to_add==-1){
                ...
                recur(root.childNodes[map[new_root_idx_arr[j]]],ans,index,goal)
            }
        }
...
function dom_extractor(dom_content,goal){
try {
    // const dom_content = fs.readFileSync('./dom_content.txt', 'utf8');
    // const goal = fs.readFileSync('./goal.txt', 'utf8');

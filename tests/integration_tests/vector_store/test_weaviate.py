...
from superagi.vector_store.weaviate import Document, Weaviate, create_weaviate_client
...
class TestWeaviate(unittest.TestCase):
    def setUp(self):
        ...
        self.weaviateVectorStore = Weaviate(
            self.client, self.embedding_model, "class_name", "text_field"
        )

    def test_get_matching_text(self):
        self.client.query.get.return_value.with_near_vector.return_value.with_where.return_value.with_limit.return_value.do.return_value = {
            "data": {"Get": {"class_name": []}}
        }
        self.embedding_model.get_embedding.return_value = "vector"
        self.weaviateVectorStore._get_metadata_fields = Mock(
            return_value=["field1", "field2"]
        )
        self.weaviateVectorStore._get_search_res = Mock(return_value="search_res")
        self.weaviateVectorStore._build_documents = Mock(
            return_value=["document1", "document2"]
        )
        self.assertEqual(
            self.weaviateVectorStore.get_matching_text(
                "query", metadata={"field1": "value"}
            ),
    ...
    def test_add_texts(self):
        ...
        self.embedding_model.get_embedding.assert_has_calls(
            [call(texts[0]), call(texts[1])]
        )
        self.assertEqual(
            self.weaviateVectorStore.add_embeddings_to_vector_db.call_count, 2
        )

    def test_add_embeddings_to_vector_db(self):
        embeddings = {
            "ids": ["id1", "id2"],
            "data_object": [{"field": "value1"}, {"field": "value2"}],
            "vectors": ["v1", "v2"],
        }
        self.weaviateVectorStore.add_embeddings_to_vector_db(embeddings)
        calls = [
            call.add_data_object(
                {"field": "value1"}, class_name="class_name", uuid="id1", vector="v1"
            ),
            call.add_data_object(
                {"field": "value2"}, class_name="class_name", uuid="id2", vector="v2"
            ),
        ]

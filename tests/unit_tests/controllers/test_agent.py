...
def test_edit_schedule_success(mock_schedule, mock_patch_schedule_input):
    with patch("superagi.controllers.agent.db") as mock_db:
        ...
        start_time = datetime.strptime(
            mock_patch_schedule_input["start_time"], "%Y-%m-%d %H:%M:%S"
        )
        expiry_date = datetime.strptime(
            mock_patch_schedule_input["expiry_date"], "%Y-%m-%d %H:%M:%S"
        )

        # Verify changes in the mock agent schedule
        assert mock_schedule.start_time == start_time
        assert (
            mock_schedule.recurrence_interval
            == mock_patch_schedule_input["recurrence_interval"]
        )
...
def test_get_schedule_data_success(mock_schedule_get, mock_agent_config):
    with patch("superagi.controllers.agent.db") as mock_db:
        ...
        expected_data = {
            ...
            "expiry_date": mock_schedule_get.expiry_date.astimezone(
                timezone("GMT")
            ).strftime("%d/%m/%Y"),
...
def test_create_and_schedule_agent_success(
    mock_agent_config_schedule, mock_agent, mock_schedule
):
    with patch("superagi.models.agent.Agent") as AgentMock, patch(
        "superagi.controllers.agent.Project"
    ) as ProjectMock, patch("superagi.controllers.agent.Tool") as ToolMock, patch(
        "superagi.controllers.agent.Toolkit"
    ) as ToolkitMock, patch(
        "superagi.controllers.agent.AgentSchedule"
    ) as AgentScheduleMock, patch(
        "superagi.controllers.agent.db"
    ) as db_mock:
        project_mock = Mock()
        ...
        db_mock.session.commit.side_effect = lambda: setattr(
            agent_schedule_mock, "id", 1
        )  # id is set after commit
        db_mock.session.query.return_value.get.return_value = project_mock

...
from superagi.controllers.api.agent import (
    AgentConfigUpdateExtInput,
    ExecutionStateChangeConfigIn,
)
...
def test_update_agent_not_found(mock_update_agent_config, mock_api_key_get):
    with patch(
        "superagi.helper.auth.get_organisation_from_api_key"
    ) as mock_get_user_org, patch(
        "superagi.helper.auth.validate_api_key"
    ) as mock_validate_api_key, patch(
        "superagi.helper.auth.db"
    ) as mock_auth_db, patch(
        "superagi.controllers.api.agent.db"
    ) as db_mock:
        # Mock the session
        ...
        response = client.put(
            "/v1/agent/1",
            headers={
                "X-API-Key": mock_api_key_get
            },  # Provide the mock API key in headers
...
def test_get_run_resources_no_run_ids(mock_run_id_config_empty, mock_api_key_get):
    with patch(
        ...
        "superagi.controllers.api.agent.get_config", return_value="S3"
    ) as mock_get_config:
        # Mock the session
        mock_session = create_autospec(Session)
        # # Configure session query methods to return None for agent
...
def test_get_run_resources_invalid_run_ids(
    mock_run_id_config_invalid, mock_api_key_get
):
    with patch(
        "superagi.helper.auth.get_organisation_from_api_key"
    ) as mock_get_user_org, patch(
...
def test_resume_agent_runs_agent_not_found(
    mock_execution_state_change_input, mock_api_key_get
):
    with patch(
        "superagi.helper.auth.get_organisation_from_api_key"
    ) as mock_get_user_org, patch(
...
def test_pause_agent_runs_agent_not_found(
    mock_execution_state_change_input, mock_api_key_get
):
    with patch(
        "superagi.helper.auth.get_organisation_from_api_key"

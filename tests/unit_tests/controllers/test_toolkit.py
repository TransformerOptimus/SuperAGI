...
def mocks():
    ...
    toolkit_1 = Toolkit(
        id=1, name="toolkit_1", description="None", show_toolkit=None, organisation_id=1
    )
    toolkit_2 = Toolkit(
        id=1, name="toolkit_2", description="None", show_toolkit=None, organisation_id=1
    )
...
def test_handle_marketplace_operations_list(mocks):
    ...
    with patch(
        "superagi.helper.auth.get_user_organisation"
    ) as mock_get_user_org, patch("superagi.controllers.toolkit.db") as mock_db, patch(
        "superagi.models.toolkit.Toolkit.fetch_marketplace_list"
    ) as mock_fetch_marketplace_list, patch(
        "superagi.helper.auth.db"
    ) as mock_auth_db:
        # Set up mock data
        mock_db.session.query.return_value.filter.return_value.all.side_effect = [
            user_toolkits
        ]
        mock_fetch_marketplace_list.return_value = [
            toolkit_1.to_dict(),
            toolkit_2.to_dict(),
        ]
...
def test_install_toolkit_from_marketplace(mock_toolkit_details):
    ...
    with patch(
        "superagi.helper.auth.get_user_organisation"
    ) as mock_get_user_org, patch(
        "superagi.models.toolkit.Toolkit.fetch_marketplace_detail"
    ) as mock_fetch_marketplace_detail, patch(
        "superagi.models.toolkit.Toolkit.add_or_update"
    ) as mock_add_or_update, patch(
        "superagi.models.tool.Tool.add_or_update"
    ) as mock_tool_add_or_update, patch(
        "superagi.controllers.toolkit.db"
    ) as mock_db, patch(
        "superagi.helper.auth.db"
    ) as mock_auth_db, patch(
        "superagi.models.tool_config.ToolConfig.add_or_update"
    ) as mock_tool_config_add_or_update:
        ...
        mock_add_or_update.return_value = Toolkit(
            id=1,
            name=mock_toolkit_details["name"],
        ...
        mock_fetch_marketplace_detail.assert_called_once_with(

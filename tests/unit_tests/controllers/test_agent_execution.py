...
def test_schedule_existing_agent_already_scheduled(
    mock_patch_schedule_input, mock_schedule
):
    with patch("superagi.controllers.agent_execution.db") as mock_db:
        ...
        response = client.post(
            "agentexecutions/schedule", json=mock_patch_schedule_input
        )

        assert response.status_code == 201
        assert mock_schedule.start_time == datetime.strptime(
        ...
        assert mock_schedule.expiry_date == datetime.strptime(

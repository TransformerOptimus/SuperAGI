...
def test_edit_agent_template_success(mock_get_user_org, mock_auth_db, mock_db):
    ...
    mock_agent_template = AgentTemplate(
        id=1, name="Test Agent Template", description="Test Description"
    )
    ...
    mock_updated_agent_configs = {
        ...
        "agent_configs": {
            ...
            "goal": [
                "Create a simple pacman game for me.",
                "Write all files properly.",
            ],
            "instruction": [
                "write spec",
                "write code",
    ...
    mock_agent_template_config = AgentTemplateConfig(
        agent_template_id=1,
        key="goal",
        value=["Create a simple pacman game for me.", "Write all files properly."],
    )

    # Call the endpoint
    response = client.put(
        "agent_templates/update_agent_template/1", json=mock_updated_agent_configs
    )
    ...
    assert mock_agent_template_config.value == [
        "Create a simple pacman game for me.",
        "Write all files properly.",
    ]
...
@patch("superagi.helper.auth.db")
@patch("superagi.helper.auth.get_user_organisation")
def test_edit_agent_template_with_new_config_success(
    mock_get_user_org, mock_auth_db, mock_db
):
    # Create a mock agent template
    mock_agent_template = AgentTemplate(
        id=1, name="Test Agent Template", description="Test Description"

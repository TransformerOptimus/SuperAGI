...
def mocks():
    ...
    mock_agent_config = AgentConfiguration(
        id=1, agent_id=1, key="test_key", value="['test']"
    )
    mock_execution = AgentExecution(id=54, agent_id=1, name="test_execution")
    mock_execution_config = [
        AgentExecutionConfiguration(
            id=64, agent_execution_id=1, key="test_key", value="['test']"
        )
    ]
...
def test_get_agent_execution_configuration_not_found_failure():
    with patch("superagi.controllers.agent_execution_config.db") as mock_db:
        ...
        response = client.get(
            "/agent_executions_configs/details/agent_id/1/agent_execution_id/1"
        )

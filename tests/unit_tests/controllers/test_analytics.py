...
def test_get_metrics_success(mock_get_db):
    with patch(
        ...
        "superagi.controllers.analytics.db"
    ) as mock_db, patch(
        "superagi.controllers.analytics.AnalyticsHelper"
    ) as mock_helper, patch(
        "superagi.helper.auth.db"
    ) as mock_auth_db:
        mock_helper().calculate_run_completed_metrics.return_value = {
            "total_tokens": 10,
            "total_calls": 5,
            "runs_completed": 2,
        }
...
def test_get_agents_success(mock_get_db):
    with patch(
        ...
        mock_helper().fetch_agent_data.return_value = {
            "agent_details": "mock_details",
...
def test_get_tools_user_success(mock_get_db):
    with patch(
        ...
        "superagi.controllers.analytics.ToolsHandler"

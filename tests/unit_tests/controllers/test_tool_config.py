...
def test_update_tool_configs_success():
    ...
    with patch(
        "superagi.models.toolkit.Toolkit.get_toolkit_from_name"
    ) as get_toolkit_from_name, patch("superagi.controllers.tool_config.db") as mock_db:
        mock_db.query.return_value.filter_by.return_value.first.side_effect = [
            # First call to query
            MagicMock(toolkit_id=1, key="config_1", value="old_value_1"),
            # Second call to query
            MagicMock(toolkit_id=1, key="config_2", value="old_value_2"),
        ]
...
def test_get_all_tool_configs_success(mocks):
    ...
    with patch(
        ...
        "superagi.controllers.tool_config.db"
    ) as mock_db, patch(
...
def test_get_tool_config_success(mocks):
    ...
    with patch(
        ...
        # Call the function
        response = client.get(
            f"/tool_configs/get/toolkit/{toolkit_1.name}/key/{tool_config.key}"
        )
...
def test_get_tool_config_unauthorized(mocks):
    ...
    with patch(
        ...
        response = client.get(
            f"/tool_configs/get/toolkit/{toolkit_2.name}/key/{tool_config.key}"
        )

...
def test_get_model_source_from_model():
    ...
    for model in open_ai_models:
        assert (
            ModelSourceType.get_model_source_from_model(model) == ModelSourceType.OpenAI
        )

    for model in google_models:
        assert (
            ModelSourceType.get_model_source_from_model(model)
            == ModelSourceType.GooglePalm
        )

    assert (
        ModelSourceType.get_model_source_from_model("unregistered-model")

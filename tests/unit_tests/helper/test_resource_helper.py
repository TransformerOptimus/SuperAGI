...
def test_make_written_file_resource(mocker):
    ...
    mocker.patch(
        "superagi.helper.resource_helper.get_config",
        side_effect=["FILE", "/", "/", "FILE"],
    )
    ...
    with patch("superagi.helper.resource_helper.logger") as logger_mock:
        ...
        result = ResourceHelper.make_written_file_resource(
            "test.txt", mock_agent, mock_agent_execution, session
        )
...
def test_get_agent_resource_path(mocker):
    ...
    mock_agent = Agent(id=1, name="TestAgent")
    mock_agent_execution = AgentExecution(id=1, name="TestExecution")
    result = ResourceHelper.get_agent_write_resource_path(
        "test.txt", mock_agent, mock_agent_execution
    )
...
def test_get_formatted_agent_execution_level_path():
    ...
    formatted_path = ResourceHelper.get_formatted_agent_execution_level_path(

...
class TestTwitterTokens(unittest.TestCase):
    ...
    @patch(
        "superagi.helper.twitter_tokens.TwitterTokens.gen_nonce", return_value=123456
    )  # Replace '__main__' with actual module name
    @patch(
        "superagi.helper.twitter_tokens.TwitterTokens.percent_encode",
        return_value="encoded",
    )  # Replace '__main__' with actual module name
    def test_get_request_token(
        self, mock_percent_encode, mock_gen_nonce, mock_https_connection, mock_time
    ):
        response_mock = Mock()
        response_mock.read.return_value = (
            b"oauth_token=test_token&oauth_token_secret=test_secret"
        )
        ...
        self.assertEqual(
            self.twitter_tokens.get_request_token(api_data), expected_result
        )

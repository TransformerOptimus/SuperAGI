...
class TestCalendarDate(unittest.TestCase):
    def setUp(self):
        ...
        self.service.calendars().get().execute.return_value = {
    ...
    def test_localize_daterange(self):
        ...
        start_datetime_utc, end_datetime_utc = self.cd._localize_daterange(
            start_date, end_date, start_time, end_time, local_tz
        )

        self.assertEqual(
            start_datetime_utc, datetime(2022, 1, 1, 4, 30, tzinfo=timezone.utc)
        )
        self.assertEqual(
            end_datetime_utc, datetime(2022, 1, 2, 6, 30, tzinfo=timezone.utc)
        )
    ...
    def test_get_date_utc(self):
        ...
        result = self.cd.get_date_utc(
            start_date, end_date, start_time, end_time, self.service
        )
    ...
    def test_create_event_dates(self):
        ...
        result = self.cd.create_event_dates(
            self.service, start_date, start_time, end_date, end_time
        )
        self.assertEqual(date_utc, result)

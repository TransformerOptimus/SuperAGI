...
def test_update_next_scheduled_time(
    mock_datetime, mock_session, mock_agent_schedule, mock_parse_interval_to_seconds
):
...
@patch(
    "superagi.helper.agent_schedule_helper.AgentScheduleHelper._AgentScheduleHelper__create_execution_name_for_scheduling"
)
@patch(
    "superagi.helper.agent_schedule_helper.AgentScheduleHelper._AgentScheduleHelper__should_execute_agent"
)
@patch(
    "superagi.helper.agent_schedule_helper.AgentScheduleHelper._AgentScheduleHelper__can_remove_agent"
)
@patch(
    "superagi.helper.agent_schedule_helper.AgentScheduleHelper._AgentScheduleHelper__execute_schedule"
)
@patch("superagi.helper.agent_schedule_helper.parse_interval_to_seconds")
...
def test_run_scheduled_agents(
    ...
    mock_should_execute_agent.assert_called_once_with(
        mock_agent, mock_agent.recurrence_interval
    )
    mock_can_remove_agent.assert_called_once_with(
        mock_agent, mock_agent.recurrence_interval
    )

    mock_execute_schedule.assert_has_calls(
        [
            call(
    ...
    mock_create_execution_name_for_scheduling.assert_called_once_with(
        mock_agent.agent_id
    )

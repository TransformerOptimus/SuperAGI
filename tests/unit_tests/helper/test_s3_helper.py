...
def test__get_s3_client(s3helper_object):
    with patch(
        "superagi.helper.s3_helper.get_config", return_value="test"
    ) as mock_get_config:
...
def test_upload_file(s3helper_object, have_creds, raises):
    ...
    s3helper_object.s3.upload_fileobj.side_effect = (
        NoCredentialsError() if not have_creds else None
    )
...
def test_get_json_file(s3helper_object, have_creds, raises):
    ...
    mock_json_file["Body"].read = MagicMock(
        return_value=bytes(json.dumps("content_of_json"), "utf-8")
    )
    ...
    if raises:
        ...
        s3helper_object.s3.get_object.assert_called_with(
            Bucket=s3helper_object.bucket_name, Key=mock_path
        )

        assert (
            content == "content_of_json"
        )  # Assert we got our mocked JSON content back
...
@pytest.mark.parametrize(
    "http_status, expected_result, raises",
    [(200, "file_content", False), (500, None, True)],
)
...
@pytest.mark.parametrize(
    "http_status, expected_result, raises",
    [(200, b"file_content", False), (500, None, True)],
)
...
def test_list_files_from_s3(s3helper_object):
    s3helper_object.s3.list_objects_v2 = MagicMock(
        return_value={

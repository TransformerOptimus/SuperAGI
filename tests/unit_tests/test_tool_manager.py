...
def test_download_and_extract_tools(mock_load_tools_config, mock_download_tool):
    ...
    mock_download_tool.assert_any_call(
        "url1", os.path.join("superagi", "tools", "external_tools", "tool1")
    )
    mock_download_tool.assert_any_call(
        "url2", os.path.join("superagi", "tools", "external_tools", "tool2")
    )

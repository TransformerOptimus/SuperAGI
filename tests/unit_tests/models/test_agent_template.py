...
def test_fetch_marketplace_list(mock_get):
    mock_get.return_value = Mock(
        status_code=200, json=lambda: [{"id": 1, "name": "test", "description": "desc"}]
    )
...
def test_eval_agent_config():
    ...
    assert AgentTemplate.eval_agent_config("goal", '["goal1", "goal2"]') == [
        "goal1",
        "goal2",
    ]
...
def test_clone_agent_template_from_marketplace(
    mock_session, mock_fetch_marketplace_detail
):
    mock_fetch_marketplace_detail.return_value = {
        ...
        "configs": {"config1": {"value": "value1"}, "config2": {"value": "value2"}},
    }
    mock_session.query.return_value.filter.return_value.first.return_value = (
        AgentWorkflow(id=1, name="workflow1")
    )

    agent_template = AgentTemplate.clone_agent_template_from_marketplace(
        mock_session, 1, 1
    )

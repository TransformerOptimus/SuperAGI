...
class TestVectordbConfigs(unittest.TestCase):
    ...
    def test_get_vector_db_config_from_db_id(self, model_mock):
        ...
        result = VectordbConfigs.get_vector_db_config_from_db_id(
            self.session_mock, self.vector_db_id_mock
        )
    ...
    def test_add_vector_db_config(self, model_mock):
        VectordbConfigs.add_vector_db_config(
            self.session_mock, self.vector_db_id_mock, self.db_creds_mock
        )
    ...
    def test_delete_vector_db_configs(self, model_mock):
        VectordbConfigs.delete_vector_db_configs(
            self.session_mock, self.vector_db_id_mock
        )
        self.assertTrue(
            self.session_mock.query(model_mock)
            .filter(model_mock.vector_db_id == self.vector_db_id_mock)
            .delete.called
        )
        self.assertTrue(self.session_mock.commit.called)

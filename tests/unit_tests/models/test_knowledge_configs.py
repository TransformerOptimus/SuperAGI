...
class TestKnowledgeConfigs(unittest.TestCase):
    ...
    def test_fetch_knowledge_config_details_marketplace(self, mock_get):
        ...
        configs = KnowledgeConfigs.fetch_knowledge_config_details_marketplace(
            self.knowledge_id
        )
        self.assertEqual(configs, self.test_configs)

    def test_add_update_knowledge_config(self):
        KnowledgeConfigs.add_update_knowledge_config(
            self.session, self.knowledge_id, self.test_configs
        )
    ...
    def test_get_knowledge_config_from_knowledge_id(self):
        ...
        configs = KnowledgeConfigs.get_knowledge_config_from_knowledge_id(
            self.session, self.knowledge_id
        )

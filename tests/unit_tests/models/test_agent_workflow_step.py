...
@patch("sqlalchemy.orm.Session.query")
@patch(
    "superagi.models.workflows.agent_workflow_step.AgentWorkflowStepTool.find_or_create_tool"
)
def test_find_or_create_tool_workflow_step(
    mock_find_or_create_tool, mock_query, mock_commit, mock_add
):
    mock_find_or_create_tool.return_value = MagicMock(id=2)
    mock_query.return_value.filter.return_value.first.return_value = (
        None  # to simulate workflow_step not exists yet
    )
...
def test_find_or_create_tool_workflow_step_exists(
    mock_find_or_create_tool, mock_query, mock_commit
):
...
@patch("sqlalchemy.orm.Session.query")
@patch(
    "superagi.models.workflows.iteration_workflow.IterationWorkflow.find_workflow_by_name"
)
def test_find_or_create_iteration_workflow_step_exists(
    mock_find_workflow_by_name, mock_query, mock_commit
):
    existing_workflow_step = MagicMock(spec=AgentWorkflowStep)
...
def test_add_next_workflow_step_existing(mock_find_by_id, mock_query, mock_commit):
    ...
    current_step = MagicMock(
        spec=AgentWorkflowStep,
        next_steps=[{"step_response": "previous_response", "step_id": "2"}],
    )
    mock_query.return_value.filter.return_value.first.return_value = current_step
    session = MagicMock(spec=Session)
    session.query = mock_query
...
@patch("superagi.models.workflows.agent_workflow_step.AgentWorkflowStep.find_by_id")
@patch(
    "superagi.models.workflows.agent_workflow_step.AgentWorkflowStep.find_by_unique_id"
)
def test_fetch_default_next_step(mock_find_by_unique_id, mock_find_by_id):
    current_step = MagicMock(
        spec=AgentWorkflowStep,
        next_steps=[{"step_response": "default", "step_id": "2"}],
    )
...
def test_fetch_default_next_step_none(mock_find_by_id):
    current_step = MagicMock(
        spec=AgentWorkflowStep,
        next_steps=[{"step_response": "non-default", "step_id": "2"}],
    )
    mock_find_by_id.return_value = current_step
    session = MagicMock(spec=Session)
    result = AgentWorkflowStep.fetch_default_next_step(

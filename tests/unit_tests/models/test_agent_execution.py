...
def test_assign_next_step_id(mock_session, mocker):
    ...
    mock_step = AgentWorkflowStep(
        id=2, action_type="ITERATION_WORKFLOW", action_reference_id=1
    )
    ...
    mocker.patch.object(
        IterationWorkflow, "fetch_trigger_step_id", return_value=mock_trigger_step
    )
...
def test_get_execution_by_agent_id_and_status():
    ...
    mock_agent_execution = AgentExecution(
        id=agent_execution_id, name="Test Execution", status="RUNNING"
    )
    ...
    # Call the method under test
    agent_execution = AgentExecution.get_execution_by_agent_id_and_status(
        session, agent_execution_id, "RUNNING"
    )

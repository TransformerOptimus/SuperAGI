...
def mock_get_agent_execution_from_id():
    with patch(
        "superagi.models.agent_execution.AgentExecution.get_agent_execution_from_id"
    ) as mock_execution:
...
def mock_resource_helper():
    with patch(
        "superagi.helper.resource_helper.ResourceHelper.get_agent_read_resource_path"
    ) as mock_resource_helper:
...
def test_read_file_tool(
    ...
    mock_get_agent_execution_from_id,
...
def test_read_file_tool_s3(
    ...
    with tempfile.NamedTemporaryFile("w", delete=False, suffix=".txt") as tmp:
        ...
        mock_resource_helper.return_value = tmp.name
        mock_s3_helper.return_value.read_from_s3.return_value = open(
            tmp.name, "r"
        ).read()

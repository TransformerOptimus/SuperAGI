...
class TestSendTweetsTool(unittest.TestCase):
    @patch(
        "superagi.helper.twitter_tokens.TwitterTokens.get_twitter_creds",
        return_value={"token": "123", "token_secret": "456"},
    )
    @patch(
        "superagi.helper.twitter_helper.TwitterHelper.get_media_ids", return_value=[789]
    )
    @patch("superagi.helper.twitter_helper.TwitterHelper.send_tweets")
    def test_execute(
        self, mock_send_tweets, mock_get_media_ids, mock_get_twitter_creds
    ):
        ...
        self.assertEqual(
            obj._execute(False, "Hello world", ["image1.png"]),
            "Error posting tweet. (Status code: 400)",
        )
        mock_get_twitter_creds.assert_called_once_with(1)
        mock_get_media_ids.assert_not_called()
        mock_send_tweets.assert_called_once_with(
            {"text": "Hello world"}, {"token": "123", "token_secret": "456"}
        )

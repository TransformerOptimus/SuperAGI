...
def test_execute_dalle_image_gen_tool(mock_config, mock_requests, mock_dalle):
    ...
    tool.toolkit_config.session.query.return_value.filter.return_value.first.return_value = MagicMock(
    ...
    mock_dalle_instance.generate_image.return_value = MagicMock(
        _previous=MagicMock(
            data=[{"url": "http://test_url1.com"}, {"url": "http://test_url2.com"}]
        )
    )
    ...
    tool.resource_manager.write_binary_file.assert_any_call(
        "image1.png", b"test_image_data"
    )
    tool.resource_manager.write_binary_file.assert_any_call(
        "image2.png", b"test_image_data"
    )
...
@patch("superagi.tools.image_generation.dalle_image_gen.requests")
@patch("superagi.tools.image_generation.dalle_image_gen.Configuration")
def test_execute_dalle_image_gen_tool_invalid_api_key(
    mock_config, mock_requests, mock_dalle
):
    tool = DalleImageGenTool()
    tool.toolkit_config = MagicMock(toolkit_id=1)

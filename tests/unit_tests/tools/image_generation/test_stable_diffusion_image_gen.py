...
def stable_diffusion_tool():
    with patch(
        "superagi.tools.image_generation.stable_diffusion_image_gen.requests.post"
    ) as post_mock, patch(
        "superagi.tools.image_generation.stable_diffusion_image_gen.FileManager"
    ) as resource_manager_mock, patch(
        "superagi.tools.image_generation.stable_diffusion_image_gen.ResourceHelper"
    ) as resource_helper_mock, patch(
        "superagi.tools.image_generation.stable_diffusion_image_gen.Agent"
    ) as agent_mock, patch(
        "superagi.tools.image_generation.stable_diffusion_image_gen.AgentExecution"
    ) as agent_execution_mock:
...
def test_execute(stable_diffusion_tool):
    ...
    tool.toolkit_config.get_tool_config = (
        lambda key: "fake_api_key" if key == "STABILITY_API_KEY" else "engine_id_1"
    )
    prompt = "Test prompt"

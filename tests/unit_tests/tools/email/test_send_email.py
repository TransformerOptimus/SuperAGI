...
def test_execute_sends_email(mock_imap_open, mock_smtp):
    ...
    # When
    result = send_email_tool._execute(
        "receiver@example.com", "test subject", "test body"
    )
...
def test_execute_sends_email_to_draft(mock_imap_open, mock_smtp):
    ...
    mock_imap_open.assert_called_once_with(
        "Drafts", "sender@example.com", "password", "imap.example.com"
    )

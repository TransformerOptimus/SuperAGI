...
def test_execute(mock_improve_code_tool):
    ...
    mock_improve_code_tool.resource_manager.write_file.return_value = (
        "file saved successfully"
    )

    assert (
        mock_improve_code_tool._execute()
        == "All codes improved and saved successfully in: test1 test2"
    )

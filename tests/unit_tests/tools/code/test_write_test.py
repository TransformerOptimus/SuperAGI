...
def test_execute(mock_token_counter, mock_agent_prompt_builder, mock_prompt_reader):
    ...
    test_tool.tool_response_manager.get_last_response.return_value = (
        "WriteSpecTool response"
    )
    mock_prompt_reader.read_tools_prompt.return_value = (
        "Prompt template {goals} {test_description} {spec}"
    )
    ...
    mock_agent_prompt_builder.add_list_items_to_string.assert_called_once_with(
        test_tool.goals
    )

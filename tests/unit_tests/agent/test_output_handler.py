...
from superagi.agent.output_handler import (
    ReplaceTaskOutputHandler,
    TaskOutputHandler,
    ToolOutputHandler,
)
...
def test_tool_output_handle(
    parse_mock, execute_mock, get_completed_tasks_mock, complete_task_mock
):
    # Arrange
    ...
    handler._check_for_completion = Mock(
        return_value=Mock(status="PENDING", is_permission_required=False)
    )
    handler.handle_tool_response = Mock(
        return_value=Mock(status="PENDING", is_permission_required=False)
    )
    # Act
...
def test_tool_handler_check_permission_in_restricted_mode(op_mock):
    ...
    assistant_reply = '{"tool": {"name": "someAction", "args": ["arg1", "arg2"]}}'
    op_mock.parse.return_value = AgentGPTAction(
        name="someAction", args=["arg1", "arg2"]
    )
    tool = MagicMock()
    ...
    # Act
    response = handler._check_permission_in_restricted_mode(
        session_mock, assistant_reply
    )
...
def test_task_output_handle_method(
    extract_json_array_section_mock, get_tasks_mock, add_task_mock
):
    # Arrange
    agent_execution_id = 1
...
def test_handle_method(
    extract_json_array_section_mock, get_tasks_mock, add_task_mock, clear_tasks_mock
):
    # Arrange
    agent_execution_id = 1
    agent_config = {}

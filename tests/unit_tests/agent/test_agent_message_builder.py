...
def test_build_agent_messages(mock_get_config, mock_token_limit):
    ...
    messages = builder.build_agent_messages(
        prompt, agent_feeds, history_enabled=True, completion_prompt=completion_prompt
    )
...
@patch(
    "superagi.models.agent_execution_config.AgentExecutionConfiguration.add_or_update_agent_execution_config"
)
@patch(
    "superagi.agent.agent_message_builder.AgentLlmMessageBuilder._build_prompt_for_recursive_ltm_summary_using_previous_ltm_summary"
)
@patch(
    "superagi.agent.agent_message_builder.AgentLlmMessageBuilder._build_prompt_for_ltm_summary"
)
@patch("superagi.helper.token_counter.TokenCounter.count_text_tokens")
@patch("superagi.helper.token_counter.TokenCounter.token_limit")
def test_build_ltm_summary(
    mock_token_limit,
    mock_count_text_tokens,
    mock_build_prompt_for_ltm_summary,
    ...
    mock_build_prompt_for_recursive_ltm_summary.return_value = (
        "recursive_ltm_summary_prompt"
    )
...
@patch("superagi.helper.prompt_reader.PromptReader.read_agent_prompt")
def test_build_prompt_for_recursive_ltm_summary_using_previous_ltm_summary(
    mock_read_agent_prompt,
):
    mock_session = Mock()
    llm = Mock()
    agent_id = 1
    ...
    mock_read_agent_prompt.return_value = (
        "{previous_ltm_summary}\n{past_messages}\n{char_limit}"
    )

    prompt = builder._build_prompt_for_recursive_ltm_summary_using_previous_ltm_summary(
        previous_ltm_summary, past_messages, token_limit
    )

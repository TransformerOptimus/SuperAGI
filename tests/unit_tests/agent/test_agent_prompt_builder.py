...
@patch(
    "superagi.agent.agent_prompt_builder.AgentPromptBuilder.add_list_items_to_string"
)
@patch("superagi.agent.agent_prompt_builder.AgentPromptBuilder.add_tools_to_prompt")
def test_replace_main_variables(
    mock_add_tools_to_prompt, mock_add_list_items_to_string
):
    super_agi_prompt = (
    ...
    result = AgentPromptBuilder.replace_main_variables(
        super_agi_prompt, goals, instructions, constraints, tools
    )
...
def test_replace_task_based_variables(mock_count_message_tokens):
    ...
    completed_tasks = [
        {"task": "task1", "response": "response1"},
        {"task": "task2", "response": "response2"},
    ]
    token_limit = 2000
    ...
    result = AgentPromptBuilder.replace_task_based_variables(
        super_agi_prompt,
        current_task,
        last_task,
        last_task_result,

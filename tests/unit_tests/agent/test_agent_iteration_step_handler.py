...
def test_build_agent_prompt(test_handler, mocker):
    ...
    mocker.patch.object(
        AgentPromptBuilder, "replace_main_variables", return_value="Test prompt"
    )
    mocker.patch.object(
        AgentPromptBuilder, "replace_task_based_variables", return_value="Test prompt"
    )
    mocker.patch.object(
        task_queue,
        "get_last_task_details",
        return_value={"task": "last task", "response": "last response"},
    )
    ...
    mocker.patch(
        "superagi.agent.agent_iteration_step_handler.get_config", return_value=600
    )

    # Act
    test_handler.task_queue = task_queue
    result_prompt = test_handler._build_agent_prompt(
        iteration_workflow, agent_config, agent_execution_config, prompt, agent_tools
    )

    # Assert
    assert result_prompt == "Test prompt"
    AgentPromptBuilder.replace_main_variables.assert_called_once_with(
        prompt,
...
def test_build_tools(test_handler, mocker):
    ...
    mocker.patch.object(
        AgentConfiguration, "get_model_api_key", return_value="test_api_key"
    )
    mocker.patch.object(ToolBuilder, "build_tool")
    mocker.patch.object(
        ToolBuilder, "set_default_params_tool", return_value=ThinkingTool()
    )
    mocker.patch.object(
        ResourceSummarizer, "fetch_or_create_agent_resource_summary", return_value=True
    )
    mocker.patch("superagi.models.tool.Tool")
    test_handler.session.query.return_value.filter.return_value.all.return_value = [
        ThinkingTool()
    ]
...
def test_handle_wait_for_permission(test_handler, mocker):
    ...
    test_handler.session.query.return_value.filter.return_value.first.return_value = (
        mock_permission
    )
    ...
    result = test_handler._handle_wait_for_permission(
        ...
        mock_iteration_workflow_step,
    )

    # Assert
    test_handler._build_tools.assert_called_once_with(
        agent_config, agent_execution_config
    )

...
@patch(
    "superagi.agent.queue_step_handler.AgentExecution"
...
def test_execute_step(
    ...
    task_queue_mock.return_value.get_status.return_value = (
        None  # Mock the get_status method on TaskQueue
    )
...
def test_add_to_queue(
    task_queue_mock, agent_execution_feed_mock, queue_step_handler, step_tool
):
    # Setup mocks
    queue_step_handler._process_input_instruction = Mock(
        return_value='{"reply": ["task1", "task2"]}'
    )
    ...
    queue_step_handler._process_reply.assert_called_once_with(
        task_queue_mock, '{"reply": ["task1", "task2"]}'
    )


@patch("superagi.agent.queue_step_handler.TaskQueue")
@patch("superagi.agent.queue_step_handler.AgentExecutionFeed")
def test_consume_from_queue(
    task_queue_mock, agent_execution_feed_mock, queue_step_handler, step_tool
):
    # Setup mocks

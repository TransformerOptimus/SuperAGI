...
def test_save_document_to_vector_store(
    mock_vc_from_docs, mock_sc_from_defaults, mock_get_vector_store
):
    ...
    # Run test method
    resource_manager.save_document_to_vector_store(
        documents, resource_id, "test_model_api_key"
    )

    # Validate calls
    mock_get_vector_store.assert_called_once()
    mock_sc_from_defaults.assert_called_once_with(vector_store=mock_vector_store)
    mock_vc_from_docs.assert_called_once_with(
        documents, storage_context="mock_storage_context"
    )

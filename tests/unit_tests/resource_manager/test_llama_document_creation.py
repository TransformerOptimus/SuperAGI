...
def test_create_llama_document_s3(mocker):
    ...
    mocker.patch(
        "superagi.resource_manager.resource_manager.get_config",
    ...
    mocker.patch(
        "superagi.resource_manager.resource_manager.SimpleDirectoryReader",
    ...
    mock_boto_client.get_object.assert_called_once_with(
        Bucket="mock_bucket", Key="mock_file_path"
    )

...
from superagi.tools.google_calendar.delete_calendar_event import (
...
class TestDeleteCalendarEventTools(unittest.TestCase):
    ...
    def test_execute_delete_event_with_valid_id(self, mock_google_calendar_creds):
        ...
        credentials_obj.get_credentials.return_value = {
            "success": True,
            "service": Mock(),
        }
        mock_google_calendar_creds.return_value = credentials_obj
        self.assertEqual(
            self.delete_tool._execute("123456"),
            "Event Successfully deleted from your Google Calendar",
        )

    @patch("your_module.GoogleCalendarCreds")
    def test_execute_delete_event_with_no_id(self, mock_google_calendar_creds):
        self.assertEqual(
            self.delete_tool._execute("None"),
            "Add Event ID to delete an event from Google Calendar",
        )
    ...
    def test_execute_delete_event_with_no_credentials(self, mock_google_calendar_creds):
        ...
        mock_google_calendar_creds.return_value = credentials_obj
        self.assertEqual(
            self.delete_tool._execute("123456"), "Kindly connect to Google Calendar"
        )

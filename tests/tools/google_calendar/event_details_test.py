...
from superagi.tools.google_calendar.event_details_calendar import (
    EventDetailsCalendarInput,
    EventDetailsCalendarTool,
)
...
class TestEventDetailsCalendarTool(unittest.TestCase):
    ...
    def test_no_event_id(self):
        with patch.object(
            ...
            self.assertEqual(
                result, "Add Event ID to fetch details of an event from Google Calendar"
            )
    ...
    def test_valid_event(self):
        ...
        with patch.object(
            ...
            with patch("your_module.base64.b64decode") as mock_b64decode:
                ...
                service.events().get.assert_called_once_with(
                    calendarId="primary", eventId="decoded_event_id"
                )
                expected_output = (
                    "Event details for the event id 'test_event_id' is - \n"
                    "Summary : Test Meeting\n"

...
class AnalyticsHelper:
    ...
    def calculate_run_completed_metrics(
        self,
    ) -> Dict[str, Dict[str, Union[int, List[Dict[str, int]]]]]:
        agent_model_query = (
            self.session.query(
                Event.event_property["model"].label("model"), Event.agent_id
            )
            .filter_by(event_name="agent_created", org_id=self.organisation_id)
            .subquery()
        )

        agent_runs_query = (
            self.session.query(
                agent_model_query.c.model, func.count(Event.id).label("runs")
            )
            .join(
                Event,
                and_(
                    Event.agent_id == agent_model_query.c.agent_id,
            ...
            .group_by(agent_model_query.c.model)
            .subquery()
        )
        ...
        agent_tokens_query = (
            self.session.query(
                ...
                func.sum(text("(event_property->>'tokens_consumed')::int")).label(
                    "tokens"
                ),
            )
            .join(
    ...
    def fetch_agent_data(self) -> Dict[str, List[Dict[str, Any]]]:
            ...
            .filter(
                and_(
                    Event.event_name.in_(
                        ["run_completed", "run_iteration_limit_crossed"]
                    ),
                    Event.org_id == self.organisation_id,
                )
            )
        ...
        tool_subquery = (
            self.session.query(
                Event.agent_id,
                func.array_agg(Event.event_property["tool_name"].distinct()).label(
                    "tools_used"
                ),
            )
        ...
        start_time_subquery = (
            ...
            .filter_by(event_name="run_created", org_id=self.organisation_id)
            .group_by(Event.agent_id, Event.event_property["agent_execution_id"])
            .subquery()
        )
        ...
        time_diff_subquery = (
            self.session.query(
                ...
                (
                    func.avg(
                        end_time_subquery.c.end_time - start_time_subquery.c.start_time
                    )
                ).label("avg_run_time"),
            )
            .join(
                end_time_subquery,
                start_time_subquery.c.agent_execution_id
                == end_time_subquery.c.agent_execution_id,
            )
            .group_by(start_time_subquery.c.agent_id)
            .subquery()
        )
        ...
        query = (
            ...
            .outerjoin(
                run_subquery, run_subquery.c.agent_id == agent_subquery.c.agent_id
            )
            .outerjoin(
                tool_subquery, tool_subquery.c.agent_id == agent_subquery.c.agent_id
            )
            .outerjoin(
                time_diff_subquery,
                time_diff_subquery.c.agent_id == agent_subquery.c.agent_id,
            )
        )
    ...
    def fetch_agent_runs(self, agent_id: int) -> List[Dict[str, int]]:
        ...
        completed_subquery = (
            self.session.query(
                Event.event_property["agent_execution_id"].label(
                    "completed_agent_execution_id"
                ),
        ...
        created_subquery = (
            self.session.query(
                Event.event_property["agent_execution_id"].label(
                    "created_agent_execution_id"
                ),
                Event.event_property["agent_execution_name"].label(
        ...
        query = self.session.query(
            ...
            completed_subquery,
            completed_subquery.c.completed_agent_execution_id
            == created_subquery.c.created_agent_execution_id,
        )
    ...
    def get_active_runs(self) -> List[Dict[str, str]]:
        ...
        agent_created_subquery = (
            self.session.query(
                Event.event_property["agent_name"].label("agent_name"), Event.agent_id
            )
            .filter_by(event_name="agent_created", org_id=self.organisation_id)
            .subquery()
        )
        ...
        ).filter(end_event_subquery.c.agent_execution_id == None)

        query = query.join(
            agent_created_subquery,
            start_subquery.c.agent_id == agent_created_subquery.c.agent_id,
        )

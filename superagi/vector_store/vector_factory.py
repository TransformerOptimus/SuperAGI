...
from superagi.vector_store import qdrant, weaviate
...
class VectorFactory:
    ...
    def get_vector_storage(
        cls, vector_store: VectorStoreType, index_name, embedding_model
    ):
        """
        ...
        if vector_store == VectorStoreType.WEAVIATE:
            ...
            client = weaviate.create_weaviate_client(
                use_embedded=use_embedded, url=url, api_key=api_key
            )
    ...
    def build_vector_storage(
        cls, vector_store: VectorStoreType, index_name, embedding_model=None, **creds
    ):
        if isinstance(vector_store, str):
            vector_store = VectorStoreType.get_vector_store_type(vector_store)
        ...
        if vector_store == VectorStoreType.PINECONE:
            try:
                pinecone.init(
                    api_key=creds["api_key"], environment=creds["environment"]
                )
                index = pinecone.Index(index_name)
        ...
        if vector_store == VectorStoreType.QDRANT:
            try:
                client = qdrant.create_qdrant_client(
                    creds["api_key"], creds["url"], creds["port"]
                )
                return qdrant.Qdrant(client, embedding_model, index_name)
            except:

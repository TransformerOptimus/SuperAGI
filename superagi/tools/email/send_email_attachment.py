...
class SendEmailAttachmentInput(BaseModel):
    ...
    body: str = Field(
        ...,
        description="Email Body to be sent, Do not add senders details in the email body and end it with Warm Regards without entering any name.",
    )
    filename: str = Field(
...
class SendEmailAttachmentTool(BaseTool):
    ...
    def _execute(self, to: str, subject: str, body: str, filename: str) -> str:
        ...
        attachment.add_header(
            "Content-Disposition", "attachment", filename=final_path.split("/")[-1]
        )
    ...
    def send_email_with_attachment(self, to, subject, body, attachment) -> str:
        ...
        if message["To"] == "example@example.com" or send_to_draft:
            ...
            conn = ImapEmail().imap_open(
                draft_folder, email_sender, email_password, imap_server
            )
            conn.append(
                draft_folder,
                "",

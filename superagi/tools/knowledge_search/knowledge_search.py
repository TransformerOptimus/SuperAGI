...
class KnowledgeSearchSchema(BaseModel):
    query: str = Field(
        ..., description="The query to search required from knowledge search"
    )
...
class KnowledgeSearchTool(BaseTool):
    ...
    def _execute(self, query: str):
        ...
        toolkit = (
            session.query(Toolkit)
            .filter(Toolkit.id == self.toolkit_config.toolkit_id)
            .first()
        )
        ...
        vector_db_index = VectordbIndices.get_vector_index_from_id(
            session, knowledge.vector_db_index_id
        )
        vector_db = Vectordbs.get_vector_db_from_id(
            session, vector_db_index.vector_db_id
        )
        db_creds = VectordbConfigs.get_vector_db_config_from_db_id(
            session, vector_db.id
        )
        model_api_key = Configuration.fetch_configuration(
            session, organisation_id, "model_api_key"
        )
        model_source = Configuration.fetch_configuration(
            session, organisation_id, "model_source"
        )
        ...
        try:
            ...
            vector_db_storage = VectorFactory.build_vector_storage(
                vector_db.db_type, vector_db_index.name, embedding_model, **db_creds
            )

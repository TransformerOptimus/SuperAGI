...
class StableDiffusionImageGenInput(BaseModel):
    prompt: str = Field(
        ...,
        description="Prompt for Image Generation to be used by Stable Diffusion. The prompt should be as descriptive as possible and mention all the details of the image to be generated",
    )
    height: int = Field(
        ..., description="Height of the image to be Generated. default height is 512"
    )
    width: int = Field(
        ..., description="Width of the image to be Generated. default width is 512"
    )
    num: int = Field(
        ..., description="Number of Images to be generated. default num is 2"
    )
    steps: int = Field(
        ..., description="Number of diffusion steps to run. default steps are 50"
    )
...
class StableDiffusionImageGenTool(BaseTool):
    ...
    def _execute(
        ...
        response = self.call_stable_diffusion(
            api_key, width, height, num, prompt, steps
        )
        ...
        for i in range(num):
            ...
            self.resource_manager.write_binary_file(
                image_names[i], img_byte_arr.getvalue()
            )

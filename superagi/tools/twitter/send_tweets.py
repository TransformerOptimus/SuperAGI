...
class SendTweetsInput(BaseModel):
    ...
    is_media: bool = Field(
        ...,
        description="'True' if there is any media to be posted with Tweet else 'False'.",
    )
    media_files: list = Field(
        ..., description="Name of the media files to be uploaded."
    )
...
class SendTweetsTool(BaseTool):
    ...
    def _execute(
        self, is_media: bool, tweet_text: str = "None", media_files: list = []
    ):
        ...
        params = {}
        if is_media:
            media_ids = TwitterHelper().get_media_ids(
                session, media_files, creds, self.agent_id, self.agent_execution_id
            )
        ...
        if tweet_response.status_code == 201:
            ...
            return "Error posting tweet. (Status code: {})".format(

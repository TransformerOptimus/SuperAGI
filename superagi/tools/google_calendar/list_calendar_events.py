...
class ListCalendarEventsInput(BaseModel):
    start_time: str = Field(
        ...,
        description="A string variable storing the start time to return events from the calendar in format 'HH:MM:SS'. if no value is given keep default value as 'None'",
    )
    start_date: str = Field(
        ...,
        description="A string variable storing the start date to return events from the calendar in format 'yyyy-mm-dd' in a string variable, if no value is given keep default value as 'None'.",
    )
    end_date: str = Field(
        ...,
        description="A string variable storing the end date to return events from the calendar in format 'yyyy-mm-dd' in a string variable, if no value is given keep default value as 'None'.",
    )
    end_time: str = Field(
        ...,
        description="A string variable storing the end time to return events from the calendar in format 'HH:MM:SS'. if no value is given keep default value as 'None'",
    )
...
class ListCalendarEventsTool(BaseTool):
    ...
    def _execute(
        ...
        date_utc = CalendarDate().get_date_utc(
            start_date, end_date, start_time, end_time, service["service"]
        )
        ...
        return (
            f"List of Google Calendar Events month successfully stored in {file_name}."
        )

    def get_google_calendar_service(self):
        return GoogleCalendarCreds(self.toolkit_config.session).get_credentials(
            self.toolkit_config.toolkit_id
        )

...
class EventDetailsCalendarInput(BaseModel):
    event_id: str = Field(
        ...,
        description="The id of event to be fetched from Google Calendar. if no value is given keep default value is None",
    )
...
class EventDetailsCalendarTool(BaseTool):
    ...
    def _execute(self, event_id: str):
        service = GoogleCalendarCreds(self.toolkit_config.session).get_credentials(
        ...
        if event_id == "None":
            ...
            eid = eid.split(" ", 1)[0]
            result = service.events().get(calendarId="primary", eventId=eid).execute()

...
class QueryResourceTool(BaseTool):
    ...
    description: str = (
        "Tool searches resources content and extracts relevant information to perform the given task."
    ...
    def _execute(self, query: str):
        ...
        vector_store_index_name = (
            self.get_tool_config(key="RESOURCE_VECTOR_STORE_INDEX_NAME")
            or "super-agent-index"
        )
        ...
        as_query_engine_args = dict(
            filters=MetadataFilters(
                filters=[ExactMatchFilter(key="agent_id", value=str(self.agent_id))]
            )
        )

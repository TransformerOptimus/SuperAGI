...
class InstagramSchema(BaseModel):
    ...
    filename: str = Field(
        ...,
        description="Name of the file to be posted. Only one file can be posted at a time.",
    )
...
class InstagramTool(BaseTool):
    ...
    def _execute(self, photo_description: str, filename: str) -> str:
        ...
        response = self.get_req_insta_id(
            root_api_url, facebook_page_id, meta_user_access_token
        )
        ...
        file_path = self.get_file_path(
            session, filename, self.agent_id, self.agent_execution_id
        )
        # handling case where image generation generates multiple images

        image_url, encoded_caption = self.get_img_url_and_encoded_caption(
            photo_description, file_path, filename
        )
        # post request for getting the media container ID
        response = self.post_media_container_id(
            root_api_url,
    ...
    def create_caption(self, photo_description: str) -> str:
        ...
        caption_prompt = caption_prompt.replace(
            "{photo_description}", str(photo_description)
        )
    ...
    def get_img_public_url(self, filename, content):
        ...
        S3Helper(get_config("INSTAGRAM_TOOL_BUCKET_NAME")).upload_file_content(
            content, object_key
        )

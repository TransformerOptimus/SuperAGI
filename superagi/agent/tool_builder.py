...
class DBToolkitConfiguration(BaseToolkitConfiguration):
    ...
    def get_tool_config(self, key: str):
        tool_config = (
            self.session.query(ToolConfig)
            .filter_by(key=key, toolkit_id=self.toolkit_id)
            .first()
        )
...
class ToolBuilder:
    ...
    def build_tool(self, tool: Tool):
        ...
        for tool_path in tool_paths:
            if os.path.exists(
                os.path.join(os.getcwd(), tool_path) + "/" + tool.folder_name
            ):
                tools_dir = tool_path
                break
        parsed_tools_dir = tools_dir.rstrip("/")
        module_name = ".".join(
            parsed_tools_dir.split("/") + [tool.folder_name, file_name]
        )
        ...
        new_object.toolkit_config = DBToolkitConfiguration(
            session=self.session, toolkit_id=tool.toolkit_id
        )
        return new_object

    def set_default_params_tool(
        self,
        tool,
        agent_config,
        ...
        if (
            ...
            and (
                ...
                or agent_config["model"] == "gpt-3.5-turbo"
            )
            and tool.name != "QueryResource"
        ):
            tool.llm = get_model(
                model="gpt-3.5-turbo", api_key=model_api_key, temperature=0.4
            )
        elif hasattr(tool, "llm"):
            tool.llm = get_model(
                model=agent_config["model"], api_key=model_api_key, temperature=0.4
            )
        ...
        if hasattr(tool, "resource_manager"):
            tool.resource_manager = FileManager(
                session=self.session,
                agent_id=self.agent_id,
                agent_execution_id=self.agent_execution_id,
            )
        if hasattr(tool, "tool_response_manager"):
            tool.tool_response_manager = ToolResponseQueryManager(
                session=self.session, agent_execution_id=self.agent_execution_id
            )

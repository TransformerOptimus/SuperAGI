...
class AgentWorkflowSeed:
    ...
    def build_sales_workflow(cls, session):
        ...
        step7 = AgentWorkflowStep.find_or_create_tool_workflow_step(
            session,
            agent_workflow.id,
        ...
        step8 = AgentWorkflowStep.find_or_create_tool_workflow_step(
            session,
            agent_workflow.id,
    ...
    def build_recruitment_workflow(cls, session):
        agent_workflow = AgentWorkflow.find_or_create_by_name(
            session, "Recruitment Workflow", "Recruitment Workflow"
        )
    ...
    def build_goal_based_agent(cls, session):
        agent_workflow = AgentWorkflow.find_or_create_by_name(
            session, "Goal Based Workflow", "Goal Based Workflow"
        )
        step1 = AgentWorkflowStep.find_or_create_iteration_workflow_step(
            session,
            agent_workflow.id,
            str(agent_workflow.id) + "_step1",
            "Goal Based Agent-I",
    ...
    def build_task_based_agent(cls, session):
        agent_workflow = AgentWorkflow.find_or_create_by_name(
            session, "Dynamic Task Workflow", "Dynamic Task Workflow"
        )
        step1 = AgentWorkflowStep.find_or_create_iteration_workflow_step(
            session,
            agent_workflow.id,
        ...
        step2 = AgentWorkflowStep.find_or_create_iteration_workflow_step(
            session,
            agent_workflow.id,
            str(agent_workflow.id) + "_step2",
            "Dynamic Task Queue-I",
            step_type="NORMAL",
        )
        AgentWorkflowStep.add_next_workflow_step(session, step1.id, step2.id)
    ...
    def build_fixed_task_based_agent(cls, session):
        agent_workflow = AgentWorkflow.find_or_create_by_name(
            session, "Fixed Task Workflow", "Fixed Task Workflow"
        )
        step1 = AgentWorkflowStep.find_or_create_iteration_workflow_step(
            session,
            agent_workflow.id,
...
class IterationWorkflowSeed:
    ...
    def build_single_step_agent(cls, session):
        iteration_workflow = IterationWorkflow.find_or_create_by_name(
            session, "Goal Based Agent-I", "Goal Based Agent"
        )
        output = AgentPromptTemplate.get_super_agi_single_prompt()
        IterationWorkflowStep.find_or_create_step(
            session,
            iteration_workflow.id,
            "gb1",
            output["prompt"],
    ...
    def build_task_based_agents(cls, session):
        iteration_workflow = IterationWorkflow.find_or_create_by_name(
            session, "Dynamic Task Queue-I", "Dynamic Task Queue", has_task_queue=True
        )
        ...
        workflow_step2 = IterationWorkflowStep.find_or_create_step(
            session,
            iteration_workflow.id,
            "tb2",
            output["prompt"],
        ...
        workflow_step3 = IterationWorkflowStep.find_or_create_step(
            session,
            iteration_workflow.id,
            "tb3",
            output["prompt"],
    ...
    def build_initialize_task_workflow(cls, session):
        iteration_workflow = IterationWorkflow.find_or_create_by_name(
            session, "Initialize Tasks-I", "Initialize Tasks", has_task_queue=True
        )
    ...
    def build_action_based_agents(cls, session):
        iteration_workflow = IterationWorkflow.find_or_create_by_name(
            session, "Fixed Task Queue-I", "Fixed Task Queue", has_task_queue=True
        )
        output = AgentPromptTemplate.analyse_task()
        IterationWorkflowStep.find_or_create_step(
            session,

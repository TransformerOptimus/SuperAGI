...
class AgentLlmMessageBuilder:
    ...
    def build_agent_messages(
        ...
        if history_enabled:
            ...
            base_token_limit = TokenCounter.count_message_tokens(
                messages, self.llm_model
            )
            full_message_history = [
                {
                    "role": agent_feed.role,
                    "content": agent_feed.feed,
                    "chat_id": agent_feed.id,
                }
            ...
            if past_messages:
                ltm_summary = self._build_ltm_summary(
    ...
    def _split_history(
        ...
        for message in reversed(history):
            ...
            if hist_token_count > pending_token_limit:
                self._add_or_update_last_agent_feed_ltm_summary_id(
                    str(history[i - 1]["chat_id"])
                )
    ...
    def _add_or_update_last_agent_feed_ltm_summary_id(
        self, last_agent_feed_ltm_summary_id
    ):
        execution = AgentExecution(id=self.agent_execution_id)
        agent_execution_configs = {
            "last_agent_feed_ltm_summary_id": last_agent_feed_ltm_summary_id
        }
        AgentExecutionConfiguration.add_or_update_agent_execution_config(
            self.session, execution, agent_execution_configs
        )

    def _build_ltm_summary(self, past_messages, output_token_limit) -> str:
        ltm_prompt = self._build_prompt_for_ltm_summary(
            past_messages=past_messages, token_limit=output_token_limit
        )

        summary = AgentExecutionConfiguration.fetch_value(
            self.session, self.agent_execution_id, "ltm_summary"
        )
        ...
        if (
            (
                TokenCounter.count_text_tokens(ltm_prompt)
                + ltm_summary_base_token_limit
                + output_token_limit
            )
            - TokenCounter.token_limit()
        ) > 0:
            ...
            past_messages = (
                self.session.query(
                    ...
                    AgentExecutionFeed.feed,
                    AgentExecutionFeed.id,
                )
                .filter(
                    AgentExecutionFeed.agent_execution_id == self.agent_execution_id,
            ...
            past_messages = [
                {
                    ...
                    "content": past_message.feed,
                    "chat_id": past_message.id,
                }
                for past_message in past_messages
            ]

            ltm_prompt = (
                self._build_prompt_for_recursive_ltm_summary_using_previous_ltm_summary(
        ...
        msgs = [
            {"role": "system", "content": "You are GPT Prompt writer"},
    ...
    def _build_prompt_for_ltm_summary(
        self, past_messages: List[BaseMessage], token_limit: int
    ):
        ltm_summary_prompt = PromptReader.read_agent_prompt(
            __file__, "agent_summary.txt"
        )
        ...
        ltm_summary_prompt = ltm_summary_prompt.replace(
            "{past_messages}", past_messages_prompt
        )

        ltm_summary_prompt = ltm_summary_prompt.replace(
            "{char_limit}", str(token_limit * 4)
        )
    ...
    def _build_prompt_for_recursive_ltm_summary_using_previous_ltm_summary(
        ...
        ltm_summary_prompt = PromptReader.read_agent_prompt(
            __file__, "agent_recursive_summary.txt"
        )

...
class AgentLlmMessageBuilder:
    ...
    def __init__(self, session, llm_model: str, agent_id: int, agent_execution_id: int, llm=None):
        ...
        if history_enabled:
            ...
            if past_messages:
                ...
                if TokenCounter.count_text_tokens(long_term_memory_prompt) - TokenCounter.token_limit() > 100:
                    last_feed_long_term_summary_id = self.session.query(AgentExecutionConfiguration).filter(
                        AgentExecutionConfiguration.agent_execution_id == self.agent_execution_id,

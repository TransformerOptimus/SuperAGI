...
class AgentToolStepHandler:
    ...
    def execute_step(self):
        ...
        assistant_reply = self._process_input_instruction(
            agent_config, agent_execution_config, step_tool, workflow_step
        )
        tool_obj = self._build_tool_obj(
            agent_config, agent_execution_config, step_tool.tool_name
        )
        ...
        if step_tool.output_instruction:
            step_response = self._process_output_instruction(
                final_response.result, step_tool, workflow_step
            )
    ...
    def _process_input_instruction(
        self, agent_config, agent_execution_config, step_tool, workflow_step
    ):
        tool_obj = self._build_tool_obj(
            agent_config, agent_execution_config, step_tool.tool_name
        )
        prompt = self._build_tool_input_prompt(
            step_tool, tool_obj, agent_execution_config
        )
    ...
    def _build_tool_obj(self, agent_config, agent_execution_config, tool_name: str):
        ...
        tool_obj = tool_builder.set_default_params_tool(
            tool_obj,
            agent_config,
    ...
    def _process_output_instruction(
        self,
        final_response: str,
        ...
        prompt = self._build_tool_output_prompt(
            step_tool, final_response, workflow_step
        )
        ...
        if "content" not in response or response["content"] is None:
            raise RuntimeError(
                f"ToolWorkflowStepHandler: Failed to get output response from llm"
            )
        total_tokens = current_tokens + TokenCounter.count_message_tokens(
            response, self.llm.get_model()
        )
    ...
    def _build_tool_input_prompt(
        ...
        super_agi_prompt = PromptReader.read_agent_prompt(
            __file__, "agent_tool_input.txt"
        )
        super_agi_prompt = super_agi_prompt.replace(
            "{goals}",
            AgentPromptBuilder.add_list_items_to_string(agent_execution_config["goal"]),
        )
    ...
    def _build_tool_output_prompt(
        ...
        super_agi_prompt = PromptReader.read_agent_prompt(
            __file__, "agent_tool_output.txt"
        )
    ...
    def _handle_wait_for_permission(
        ...
        if agent_execution_permission.status == "APPROVED":
            next_step = AgentWorkflowStep.fetch_next_step(
                self.session, workflow_step.id, "YES"
            )
        else:
            next_step = AgentWorkflowStep.fetch_next_step(
                self.session, workflow_step.id, "NO"
            )

...
class AgentIterationStepHandler:
    ...
    def __init__(
        self, session, llm, agent_id: int, agent_execution_id: int, memory=None
    ):
    ...
    def execute_step(self):
        ...
        iteration_workflow_step = IterationWorkflowStep.find_by_id(
            self.session, execution.iteration_workflow_step_id
        )
        agent_execution_config = AgentExecutionConfiguration.fetch_configuration(
            self.session, self.agent_execution_id
        )

        if not self._handle_wait_for_permission(
            execution, agent_config, agent_execution_config, iteration_workflow_step
        ):
            return
        ...
        iteration_workflow = IterationWorkflow.find_by_id(
        ...
        messages = AgentLlmMessageBuilder(
            ...
            prompt,
            agent_feeds,
            history_enabled=iteration_workflow_step.history_enabled,
        ...
        total_tokens = current_tokens + TokenCounter.count_message_tokens(
            response["content"], self.llm.get_model()
        )
        ...
        output_handler = get_output_handler(
            iteration_workflow_step.output_type,
        ...
        if response.status == "COMPLETE":
            ...
            self._update_agent_execution_next_step(
                execution, iteration_workflow_step.next_step_id, "COMPLETE"
            )
            EventHandler(session=self.session).create_event(
                "run_completed",
                {
            ...
            self._update_agent_execution_next_step(
                execution, iteration_workflow_step.next_step_id
            )
            logger.info(
                f"Starting next job for agent execution id: {self.agent_execution_id}"
            )

        self.session.flush()

    def _update_agent_execution_next_step(
        self, execution, next_step_id, step_response: str = "default"
    ):
        if next_step_id == -1:
            next_step = AgentWorkflowStep.fetch_next_step(
                self.session, execution.current_agent_step_id, step_response
            )
            if str(next_step) == "COMPLETE":
                execution.current_agent_step_id = -1
                execution.status = "COMPLETED"
            else:
                AgentExecution.assign_next_step_id(
                    self.session, self.agent_execution_id, next_step.id
                )
        else:
    ...
    def _build_agent_prompt(
        self,
        iteration_workflow: IterationWorkflow,
        agent_config: dict,
        ...
        if iteration_workflow.has_task_queue:
            ...
            prompt = AgentPromptBuilder.replace_task_based_variables(
                ...
                self.task_queue.get_completed_tasks(),
                token_limit,
            )
        return prompt
    ...
    def _build_tools(self, agent_config: dict, agent_execution_config: dict):
        ...
        model_api_key = AgentConfiguration.get_model_api_key(
            self.session, self.agent_id, agent_config["model"]
        )
        tool_builder = ToolBuilder(self.session, self.agent_id, self.agent_execution_id)
        resource_summary = ResourceSummarizer(
            session=self.session, agent_id=self.agent_id
        ...
        user_tools = (
            ...
            .filter(
                and_(Tool.id.in_(agent_config["tools"]), Tool.file_name is not None)
            )
            .all()
        )
        ...
        agent_tools = [
            tool_builder.set_default_params_tool(
                tool,
                agent_config,
    ...
    def _handle_wait_for_permission(
        self,
        agent_execution,
        agent_config: dict,
        agent_execution_config: dict,
        ...
        agent_execution_permission = (
            self.session.query(AgentExecutionPermission)
            .filter(AgentExecutionPermission.id == agent_execution.permission_id)
            .first()
        )
        ...
        if agent_execution_permission.status == "APPROVED":
            ...
            tool_output_handler = ToolOutputHandler(
                self.agent_execution_id, agent_config, agent_tools
            )
            tool_result = tool_output_handler.handle_tool_response(
                self.session, agent_execution_permission.assistant_reply
            )
            result = tool_result.result
        else:
            result = (
                f"User denied the permission to run the tool {agent_execution_permission.tool_name}"
        ...
        agent_execution_feed = AgentExecutionFeed(
            agent_execution_id=agent_execution_permission.agent_execution_id,
        ...
        agent_execution_feed1 = AgentExecutionFeed(
            agent_execution_id=agent_execution_permission.agent_execution_id,
            agent_id=agent_execution_permission.agent_id,
            feed=result,
            role="user",
        ...
        execution = AgentExecution.find_by_id(
            self.session, agent_execution_permission.agent_execution_id
        )
        self._update_agent_execution_next_step(
            execution, iteration_workflow_step.next_step_id
        )

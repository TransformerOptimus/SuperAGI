...
class MarketPlaceStats(DBBaseModel):
    ...
    def __repr__(self):
        ...
        """
        return (
            f"Knowledge(id={self.id}, reference_id='{self.reference_id}', reference_name='{self.reference_name}', "
    ...
    def update_knowledge_install_number(cls, session, knowledge_id, install_number):
        knowledge_install_number = (
            session.query(MarketPlaceStats)
            .filter(
        ...
        if knowledge_install_number is None:
            knowledge_install_number = MarketPlaceStats(
                reference_id=knowledge_id,
                reference_name="KNOWLEDGE",
                key="download_count",
                value=str(install_number),
            )

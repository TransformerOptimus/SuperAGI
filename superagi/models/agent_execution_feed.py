...
class AgentExecutionFeed(DBBaseModel):
    ...
    def get_last_tool_response(
        cls, session: Session, agent_execution_id: int, tool_name: str = None
    ):
        agent_execution_feeds = (
            session.query(AgentExecutionFeed)
            .filter(
                AgentExecutionFeed.agent_execution_id == agent_execution_id,
                AgentExecutionFeed.role == "system",
            )
            .order_by(AgentExecutionFeed.created_at.desc())
            .all()
        )
    ...
    def fetch_agent_execution_feeds(cls, session, agent_execution_id: int):
        ...
        agent_feeds = (
            session.query(
                AgentExecutionFeed.role, AgentExecutionFeed.feed, AgentExecutionFeed.id
            )
            .filter(
                AgentExecutionFeed.agent_execution_id == agent_execution_id,
                AgentExecutionFeed.feed_group_id
                == agent_execution.current_feed_group_id,
            )

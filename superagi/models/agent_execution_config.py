...
class AgentExecutionConfiguration(DBBaseModel):
    ...
    def __repr__(self):
        ...
        return (
            f"AgentExecutionConfig(id={self.id}, agent_execution_id='{self.agent_execution_id}', "
            f"key='{self.key}', value='{self.value}')"
        )

    @classmethod
    def add_or_update_agent_execution_config(
        cls, session, execution, agent_execution_configs
    ):
        agent_execution_configurations = [
            AgentExecutionConfiguration(
                agent_execution_id=execution.id, key=key, value=str(value)
            )
    ...
    def fetch_configuration(cls, session, execution_id):
        ...
        agent_configurations = (
            session.query(AgentExecutionConfiguration)
            .filter_by(agent_execution_id=execution_id)
            .all()
        )
        parsed_config = {
            "goal": [],
    ...
    @classmethod
    def build_agent_execution_config(
        cls,
        session,
        agent,
        results_agent,
        results_agent_execution,
        total_calls,
        total_tokens,
    ):
        results_agent_dict = {result.key: result.value for result in results_agent}
        results_agent_execution_dict = {
            result.key: result.value for result in results_agent_execution
        }
        ...
        if "toolkits" in results_agent_dict:
            results_agent_dict["toolkits"] = list(
                ast.literal_eval(results_agent_dict["toolkits"])
            )

        if "tools" in results_agent_dict:
            results_agent_dict["tools"] = list(
                ast.literal_eval(results_agent_dict["tools"])
            )
            tools = (
                session.query(Tool)
                .filter(Tool.id.in_(results_agent_dict["tools"]))
                .all()
            )
        ...
        knowledge_name = ""
        if (
            "knowledge" in results_agent_dict
            and results_agent_dict["knowledge"] != "None"
        ):
            if type(results_agent_dict["knowledge"]) == int:
                results_agent_dict["knowledge"] = int(results_agent_dict["knowledge"])
            knowledge = (
                session.query(Knowledges)
                .filter(Knowledges.id == results_agent_dict["knowledge"])
                .first()
            )
    ...
    @classmethod
    def build_scheduled_agent_execution_config(
        cls, session, agent, results_agent, total_calls, total_tokens
    ):
        results_agent_dict = {result.key: result.value for result in results_agent}

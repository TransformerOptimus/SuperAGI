...
class KnowledgeConfigs(DBBaseModel):
    ...
    def fetch_knowledge_config_details_marketplace(cls, knowledge_id: int):
        ...
        response = requests.get(
            marketplace_url
            + f"/knowledge_configs/marketplace/details/{str(knowledge_id)}",
            headers=headers,
            timeout=10,
        )
    ...
    def get_knowledge_config_from_knowledge_id(cls, session, knowledge_id):
        knowledge_configs = (
            session.query(KnowledgeConfigs)
    ...
    def delete_knowledge_config(cls, session, knowledge_id):
        session.query(KnowledgeConfigs).filter(
            KnowledgeConfigs.knowledge_id == knowledge_id
        ).delete()
        session.commit()

...
class VectordbIndices(DBBaseModel):
    ...
    def delete_vector_db_index(cls, session, vector_index_id):
        session.query(VectordbIndices).filter(
            VectordbIndices.id == vector_index_id
        ).delete()
        session.commit()

    @classmethod
    def add_vector_index(
        cls, session, index_name, vector_db_id, state, dimensions=None
    ):  # will be none only in the case of weaviate
        vector_index = VectordbIndices(
            name=index_name,
    ...
    def update_vector_index_state(cls, session, index_id, state):
        vector_index = (
            session.query(VectordbIndices)
            .filter(VectordbIndices.id == index_id)
            .first()
        )

...
class Knowledges(DBBaseModel):
    ...
    def __repr__(self):
        ...
        """
        return (
            f"Knowledge(id={self.id}, name='{self.name}', description='{self.description}', "
    ...
    @classmethod
    def get_knowledge_install_details(
        cls, session, marketplace_knowledges, organisation
    ):
        installed_knowledges = (
            session.query(Knowledges)
            .filter(Knowledges.organisation_id == organisation.id)
            .all()
        )
        for knowledge in marketplace_knowledges:
            if knowledge["name"] in [
                installed_knowledge.name for installed_knowledge in installed_knowledges
            ]:
    ...
    def get_knowledge_from_id(cls, session, knowledge_id):
        knowledge = (
            session.query(Knowledges).filter(Knowledges.id == knowledge_id).first()
        )
        return knowledge
    ...
    def add_update_knowledge(cls, session, knowledge_data):
        knowledge = (
            session.query(Knowledges)
            .filter(
                Knowledges.id == knowledge_data["id"],
                Knowledges.organisation_id == knowledge_data["organisation_id"],
            )
            .first()
        )
        if knowledge:
            ...
            knowledge = Knowledges(
                ...
                vector_db_index_id=knowledge_data["index_id"],
                organisation_id=knowledge_data["organisation_id"],
    ...
    def delete_knowledge_from_vector_index(cls, session, vector_db_index_id):
        session.query(Knowledges).filter(
            Knowledges.vector_db_index_id == vector_db_index_id
        ).delete()
        session.commit()

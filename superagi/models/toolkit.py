...
class Toolkit(DBBaseModel):
    ...
    def __repr__(self):
        return (
            f"ToolKit(id={self.id}, name='{self.name}', description='{self.description}', "
    ...
    @staticmethod
    def add_or_update(
        session, name, description, show_toolkit, organisation_id, tool_code_link
    ):
        # Check if the toolkit exists
        toolkit = (
            session.query(Toolkit)
            .filter(Toolkit.name == name, Toolkit.organisation_id == organisation_id)
            .first()
        )
    ...
    def fetch_tool_ids_from_toolkit(cls, session, toolkit_ids):
        ...
        for toolkit_id in toolkit_ids:
            toolkit_tools = (
                session.query(Tool).filter(Tool.toolkit_id == toolkit_id).all()
            )
    ...
    @classmethod
    def get_tool_and_toolkit_arr(
        cls, session, organisation_id: int, agent_config_tools_arr: list
    ):
        from superagi.models.tool import Tool
        ...
        for tool_obj in agent_config_tools_arr:
            toolkit = (
                session.query(Toolkit)
                .filter(
                    Toolkit.name == tool_obj["name"].strip(),
                    Toolkit.organisation_id == organisation_id,
                )
                .first()
            )
            ...
            if tool_obj.get("tools"):
                for tool_name_str in tool_obj["tools"]:
                    ...
                    if tool_db_obj is None:
                        raise Exception(
                            "One or more of the Tool(s)/Toolkit(s) does not exist."
                        )

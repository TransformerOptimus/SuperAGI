...
@app.task(
    name="initialize-schedule-agent",
...
def summarize_resource(agent_id: int, resource_id: int):
    ...
    session = Session()
    model_source = (
        Configuration.fetch_value_by_agent_id(session, agent_id, "model_source")
        or "OpenAi"
    )
    ...
    resource_summarizer.add_to_vector_store_and_create_summary(
        resource_id=resource_id, documents=documents
    )
    session.close()
...
def webhook_callback(agent_execution_id, val, old_val):
    ...
    with Session() as session:
        WebHookManager(session).agent_status_change_callback(
            agent_execution_id, val, old_val
        )

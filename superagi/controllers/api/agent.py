...
from superagi.controllers.types.agent_with_config import (
    AgentConfigExtInput,
    AgentConfigUpdateExtInput,
)
from superagi.helper.auth import get_organisation_from_api_key, validate_api_key
...
def create_agent_with_config(
    ...
    try:
        tools_arr = Toolkit.get_tool_and_toolkit_arr(
            db.session, organisation.id, agent_with_config.tools
        )
    ...
    if agent_with_config.schedule is not None:
        agent_schedule = AgentSchedule.save_schedule_from_config(
            db.session, db_agent, agent_with_config.schedule
        )
...
def create_run(
    ...
    if db_schedule is not None:
        raise HTTPException(
            status_code=409, detail="Agent is already scheduled,cannot run"
        )
    start_step_id = AgentWorkflow.fetch_trigger_step_id(
        db.session, agent.agent_workflow_id
    )
    db_agent_execution = AgentExecution.get_execution_by_agent_id_and_status(
        db.session, agent_id, "CREATED"
    )
    ...
    return {"run_id": db_agent_execution.id}


@router.put("/{agent_id}", status_code=200)
def update_agent(
    agent_id: int,
    agent_with_config: AgentConfigUpdateExtInput,
    api_key: str = Security(validate_api_key),
    organisation: Organisation = Depends(get_organisation_from_api_key),
):
    ...
    if db_schedule is not None:
        raise HTTPException(
            status_code=409, detail="Agent is already scheduled,cannot update"
        )
...
def get_agent_runs(
    agent_id: int,
    filter_config: RunFilterConfigIn,
    api_key: str = Security(validate_api_key),
    organisation: Organisation = Depends(get_organisation_from_api_key),
):
    ...
    db_execution_arr = AgentExecution.get_all_executions_by_filter_config(
        db.session, agent.id, filter_config
    )
...
def pause_agent_runs(
    agent_id: int,
    execution_state_change_input: ExecutionStateChangeConfigIn,
    api_key: str = Security(validate_api_key),
    organisation: Organisation = Depends(get_organisation_from_api_key),
):
    agent = Agent.get_active_agent_by_id(db.session, agent_id)
    ...
    if execution_state_change_input.run_ids is not None:
        try:
            AgentExecution.validate_run_ids(
                db.session, execution_state_change_input.run_ids, organisation.id
            )
        except Exception as e:
            raise HTTPException(status_code=404, detail="One or more run_ids not found")

    db_execution_arr = AgentExecution.get_all_executions_by_status_and_agent_id(
        db.session, agent.id, execution_state_change_input, "RUNNING"
    )
    for ind_execution in db_execution_arr:
...
def resume_agent_runs(
    ...
    db_execution_arr = AgentExecution.get_all_executions_by_status_and_agent_id(
        db.session, agent.id, execution_state_change_input, "PAUSED"
    )
    for ind_execution in db_execution_arr:
...
def get_run_resources(
    run_id_config: RunIDConfig,
    api_key: str = Security(validate_api_key),
    organisation: Organisation = Depends(get_organisation_from_api_key),
):
    if get_config("STORAGE_TYPE") != "S3":
        raise HTTPException(
            status_code=400, detail="This endpoint only works when S3 is configured"
        )
    run_ids_arr = run_id_config.run_ids
    if len(run_ids_arr) == 0:
        raise HTTPException(status_code=404, detail=f"No execution_id found")
    # Checking if the run_ids whose output files are requested belong to the organisation
    try:

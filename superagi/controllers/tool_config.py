...
from superagi.helper.encyption_helper import decrypt_data, encrypt_data, is_encrypted
...
@router.post(
    "/create-or-update/{toolkit_name}", status_code=201, response_model=ToolConfigOut
)
def create_or_update_tool_config(
    toolkit_name: str, tool_configs, Authorize: AuthJWT = Depends(check_auth)
):
    """
    ...
    for tool_config in tool_configs:
        existing_tool_config = (
            db.session.query(ToolConfig)
            .filter(
                ToolConfig.toolkit_id == toolkit.id, ToolConfig.key == tool_config.key
            )
            .first()
        )
        ...
        if existing_tool_config.value:
            ...
            new_tool_config = ToolConfig(
                key=tool_config.key,
                value=encrypt_data(tool_config.value),
...
def get_all_tool_configs(
    ...
    toolkit = (
        ...
        .filter(
            Toolkit.name == toolkit_name, Toolkit.organisation_id == organisation.id
        )
        .first()
    )

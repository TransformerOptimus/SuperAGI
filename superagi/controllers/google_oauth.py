...
async def google_auth_calendar(code: str = Query(...), state: str = Query(...)):
    ...
    client_id = (
        ...
        .filter(
            ToolConfig.key == "GOOGLE_CLIENT_ID", ToolConfig.toolkit_id == toolkit_id
        )
        .first()
    )
    ...
    client_secret = (
        db.session.query(ToolConfig)
        .filter(
            ToolConfig.key == "GOOGLE_CLIENT_SECRET",
            ToolConfig.toolkit_id == toolkit_id,
        )
        .first()
    )
...
def send_google_calendar_configs(
    google_creds: dict, toolkit_id: int, Authorize: AuthJWT = Depends(check_auth)
):
    engine = connect_db()
...
def get_google_calendar_tool_configs(toolkit_id: int):
    google_calendar_config = (
        db.session.query(ToolConfig)
        .filter(
            ToolConfig.toolkit_id == toolkit_id, ToolConfig.key == "GOOGLE_CLIENT_ID"
        )
        .first()
    )

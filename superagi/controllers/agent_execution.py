...
def create_agent_execution(
    agent_execution: AgentExecutionIn, Authorize: AuthJWT = Depends(check_auth)
):
    """
    ...
    agent = (
        ...
        .filter(Agent.id == agent_execution.agent_id, Agent.is_deleted == False)
        .first()
    )
    if not agent:
    ...
    agent_configs = (
        ...
        .filter(AgentConfiguration.agent_id == agent_execution.agent_id)
        .all()
    )
        ...
        if agent_config.key not in keys_to_exclude:
            if agent_config.key == "toolkits":
                if agent_config.value:
                    toolkits = [
                        int(item)
                        for item in agent_config.value.strip("{}").split(",")
...
def create_agent_run(
    agent_execution: AgentRunIn, Authorize: AuthJWT = Depends(check_auth)
):
    """
    ...
    AgentConfiguration.update_agent_configurations_table(
        session=db.session,
        agent_id=agent_execution.agent_id,
        updated_details=agent_execution,
    )
...
def schedule_existing_agent(
    agent_schedule: AgentScheduleInput, Authorize: AuthJWT = Depends(check_auth)
):
    """
    ...
    scheduled_agent = (
        ...
        .filter(
            AgentSchedule.agent_id == agent_schedule.agent_id,
...
def list_running_agents(agent_id: str, Authorize: AuthJWT = Depends(check_auth)):
    ...
    for execution in executions:
        execution.time_difference = get_time_difference(
            execution.last_execution_time, str(datetime.now())
        )
    return executions

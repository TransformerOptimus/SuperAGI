...
def send_twitter_tool_configs(
    twitter_creds: str, Authorize: AuthJWT = Depends(check_auth)
):
    ...
    toolkit = (
        ...
        .filter(Toolkit.id == credentials["toolkit_id"])
        .first()
    )
    api_key = (
        db.session.query(ToolConfig)
        .filter(
            ToolConfig.key == "TWITTER_API_KEY",
            ToolConfig.toolkit_id == credentials["toolkit_id"],
        )
        .first()
    )
    ...
    api_key_secret = (
        db.session.query(ToolConfig)
        .filter(
            ToolConfig.key == "TWITTER_API_SECRET",
            ToolConfig.toolkit_id == credentials["toolkit_id"],
        )
        .first()
    )
    ...
    tokens = OauthTokens().add_or_update(
        db.session,
        credentials["toolkit_id"],
        user_id,
        toolkit.organisation_id,
...
def get_twitter_tool_configs(toolkit_id: int):
    twitter_config_key = (
        db.session.query(ToolConfig)
        .filter(
            ToolConfig.toolkit_id == toolkit_id, ToolConfig.key == "TWITTER_API_KEY"
        )
        .first()
    )
    twitter_config_secret = (
        db.session.query(ToolConfig)
        .filter(
            ToolConfig.toolkit_id == toolkit_id, ToolConfig.key == "TWITTER_API_SECRET"
        )
        .first()
    )

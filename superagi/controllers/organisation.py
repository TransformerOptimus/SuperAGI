...
def create_organisation(organisation: sqlalchemy_to_pydantic(Organisation, exclude=["id"]), Authorize: AuthJWT = Depends()):
...
def update_organisation(organisation_id: int, organisation: sqlalchemy_to_pydantic(Organisation,exclude=["id"]),Authorize: AuthJWT = Depends()):
    db_organisation = db.session.query(Organisation).filter(Organisation.id == organisation_id).first()
    if not db_organisation:
        raise HTTPException(status_code=404, detail="Organisation not found")

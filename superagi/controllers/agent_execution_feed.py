...
def create_agent_execution_feed(
    agent_execution_feed: AgentExecutionFeedIn, Authorize: AuthJWT = Depends(check_auth)
):
    """
...
def get_agent_execution_feed(
    ...
    execution_permissions = (
        db.session.query(AgentExecutionPermission)
        .filter_by(agent_execution_id=agent_execution_id)
        .order_by(asc(AgentExecutionPermission.created_at))
        .all()
    )
    ...
    permissions = [
        {
            ...
            "time_difference": get_time_difference(
                permission.created_at, str(datetime.now())
            ),
        }

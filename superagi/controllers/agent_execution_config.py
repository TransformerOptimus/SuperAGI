...
def get_agent_execution_configuration(
    ...
    agent = (
        ...
        .filter(agent_id == Agent.id, or_(Agent.is_deleted == False))
        .first()
    )
    if not agent:
    ...
    if agent_execution_id != -1:
        agent_execution_config = AgentExecution.get_agent_execution_from_id(
    ...
    response = {}
    if agent_execution_id != -1:
        response = AgentExecutionConfiguration.build_agent_execution_config(
            db.session,
            agent,
            results_agent,
            results_agent_execution,
        ...
        response = AgentExecutionConfiguration.build_scheduled_agent_execution_config(
            db.session, agent, results_agent, total_calls, total_tokens
        )

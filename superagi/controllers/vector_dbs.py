...
def get_user_connected_vector_db_list(organisation=Depends(get_user_organisation)):
    ...
    if vector_db_list:
        for vector in vector_db_list:
            vector.updated_at = get_time_difference(
                vector.updated_at, str(datetime.now())
            )
...
def get_vector_db_details(vector_db_id: int):
    ...
    vector_db_config = VectordbConfigs.get_vector_db_config_from_db_id(
        db.session, vector_db_id
    )
    vector_db_with_config = vector_db_data | vector_db_config
    indices = (
        db.session.query(VectordbIndices)
...
def delete_vector_db(vector_db_id: int):
    try:
        vector_indices = VectordbIndices.get_vector_indices_from_vectordb(
            db.session, vector_db_id
        )
...
def connect_qdrant_vector_db(data: dict, organisation=Depends(get_user_organisation)):
    db_creds = {"api_key": data["api_key"], "url": data["url"], "port": data["port"]}
    for collection in data["collections"]:
        try:
            vector_db_storage = VectorFactory.build_vector_storage(
                "qdrant", collection, **db_creds
            )
            db_connect_for_index = vector_db_storage.get_index_stats()
            index_state = (
                "Custom" if db_connect_for_index["vector_count"] > 0 else "None"
    ...
    qdrant_db = Vectordbs.add_vector_db(
        db.session, data["name"], "Qdrant", organisation
    )
...
def connect_weaviate_vector_db(data: dict, organisation=Depends(get_user_organisation)):
    db_creds = {"api_key": data["api_key"], "url": data["url"]}
    for collection in data["collections"]:
        try:
            vector_db_storage = VectorFactory.build_vector_storage(
                "weaviate", collection, **db_creds
            )
            db_connect_for_index = vector_db_storage.get_index_stats()
            index_state = (
                "Custom" if db_connect_for_index["vector_count"] > 0 else "None"
    ...
    weaviate_db = Vectordbs.add_vector_db(
        db.session, data["name"], "Weaviate", organisation
    )
    VectordbConfigs.add_vector_db_config(db.session, weaviate_db.id, db_creds)
    for collection in data["collections"]:
        VectordbIndices.add_vector_index(
            db.session, collection, weaviate_db.id, index_state
        )
...
def update_vector_db(new_indices: list, vector_db_id: int):
    vector_db = Vectordbs.get_vector_db_from_id(db.session, vector_db_id)
    existing_indices = VectordbIndices.get_vector_indices_from_vectordb(
        db.session, vector_db_id
    )
    ...
    for index in added_indices:
        db_creds = VectordbConfigs.get_vector_db_config_from_db_id(
            db.session, vector_db_id
        )
        try:
            vector_db_storage = VectorFactory.build_vector_storage(
                vector_db.db_type, index, **db_creds
            )
            vector_db_index_stats = vector_db_storage.get_index_stats()
            index_state = (
                "Custom" if vector_db_index_stats["vector_count"] > 0 else "None"
            )
            dimensions = (
                vector_db_index_stats["dimensions"]
                if "dimensions" in vector_db_index_stats
                else None
            )
        except:
            raise HTTPException(status_code=400, detail="Unable to update vector db")
        VectordbIndices.add_vector_index(
            db.session, index, vector_db_id, index_state, dimensions
        )

...
def get_knowledge_list(
    ...
    marketplace_knowledges_with_install = Knowledges.get_knowledge_install_details(
        db.session, marketplace_knowledges, organisation
    )
    for knowledge in marketplace_knowledges_with_install:
        knowledge[
            "install_number"
        ] = MarketPlaceStats.get_knowledge_installation_number(knowledge["id"])
...
def get_marketplace_knowledge_list(page: int = 0):
    ...
    query = db.session.query(Knowledges).filter(
...
def get_user_knowledge_list(organisation=Depends(get_user_organisation)):
    ...
    user_knowledge_list = Knowledges.get_organisation_knowledges(
        db.session, organisation
    )
    for user_knowledge in user_knowledge_list:
        if user_knowledge["name"] in [
...
def get_knowledge_details(knowledge_name: str):
    ...
    knowledge_config_data = KnowledgeConfigs.fetch_knowledge_config_details_marketplace(
        knowledge_data["id"]
    )
    knowledge_data_with_config = knowledge_data | knowledge_config_data
    knowledge_data_with_config[
        "install_number"
    ] = MarketPlaceStats.get_knowledge_installation_number(
        knowledge_data_with_config["id"]
    )
    update_time = str(knowledge_data_with_config["updated_at"])
    update_time = datetime.strptime(update_time, "%Y-%m-%dT%H:%M:%S.%f")
    knowledge_data_with_config["updated_at"] = datetime.strftime(
        update_time, "%d %B %Y"
    )
...
def get_user_knowledge_details(knowledge_id: int):
    ...
    vector_database_index = VectordbIndices.get_vector_index_from_id(
        db.session, knowledge_data.vector_db_index_id
    )
    vector_database = Vectordbs.get_vector_db_from_id(
        db.session, vector_database_index.vector_db_id
    )
    knowledge = {
    ...
    knowledge_config = KnowledgeConfigs.get_knowledge_config_from_knowledge_id(
        db.session, knowledge_id
    )
...
def add_update_user_knowledge(
    knowledge_data: dict, organisation=Depends(get_user_organisation)
):
...
def install_selected_knowledge(
    knowledge_name: str,
    vector_db_index_id: int,
    organisation=Depends(get_user_organisation),
):
    vector_db_index = VectordbIndices.get_vector_index_from_id(
        db.session, vector_db_index_id
    )
    selected_knowledge = Knowledges.fetch_knowledge_details_marketplace(knowledge_name)
    selected_knowledge_config = (
        KnowledgeConfigs.fetch_knowledge_config_details_marketplace(
            selected_knowledge["id"]
        )
    )
    ...
    upsert_data = VectorEmbeddingFactory.build_vector_storage(
        vector.db_type, file_chunks
    ).get_vector_embeddings_from_chunks()
    try:
        vector_db_storage = VectorFactory.build_vector_storage(
            vector.db_type, vector_db_index.name, **db_creds
        )
    ...
    VectordbIndices.update_vector_index_state(
        db.session, vector_db_index_id, "Marketplace"
    )
    install_number = MarketPlaceStats.get_knowledge_installation_number(
        selected_knowledge["id"]
    )
    MarketPlaceStats.update_knowledge_install_number(
        db.session, selected_knowledge["id"], int(install_number) + 1
    )


@router.post("/uninstall/{knowledge_name}")
def uninstall_selected_knowledge(
    knowledge_name: str, organisation=Depends(get_user_organisation)
):
    knowledge = (
        db.session.query(Knowledges)
        .filter(
            Knowledges.name == knowledge_name,
            Knowledges.organisation_id == organisation.id,
        )
        .first()
    )
    knowledge_config = KnowledgeConfigs.get_knowledge_config_from_knowledge_id(
        db.session, knowledge.id
    )
    vector_ids = eval(knowledge_config["vector_ids"])
    vector_db_index = VectordbIndices.get_vector_index_from_id(
        db.session, knowledge.vector_db_index_id
    )
    vector = Vectordbs.get_vector_db_from_id(db.session, vector_db_index.vector_db_id)
    db_creds = VectordbConfigs.get_vector_db_config_from_db_id(db.session, vector.id)
    try:

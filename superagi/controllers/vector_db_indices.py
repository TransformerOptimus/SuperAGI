...
def get_marketplace_valid_indices(
    ...
    knowledge_with_config = KnowledgeConfigs.fetch_knowledge_config_details_marketplace(
        knowledge["id"]
    )
    pinecone = []
    qdrant = []
    ...
    for vector_db in vector_dbs:
        indices = VectordbIndices.get_vector_indices_from_vectordb(
            db.session, vector_db.id
        )
        for index in indices:
            data = {"id": index.id, "name": index.name}
            data["is_valid_dimension"] = (
                True
                if index.dimensions == int(knowledge_with_config["dimensions"])
                else False
            )

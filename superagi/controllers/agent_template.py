...
def get_agent_template(
    template_source, agent_template_id: int, organisation=Depends(get_user_organisation)
):
    """
    ...
    if template_source == "local":
        ...
        for agent_template_config in agent_template_configs:
            config_value = AgentTemplate.eval_agent_config(
                agent_template_config.key, agent_template_config.value
            )
...
def edit_agent_template(
    ...
    agent_workflow = AgentWorkflow.find_by_name(
        db.session, updated_agent_configs["agent_configs"]["agent_workflow"]
    )
    ...
    for key, value in agent_config_values.items():
        ...
        if config is not None:
            ...
            new_config = AgentTemplateConfig(
                agent_template_id=agent_template_id, key=key, value=value
            )
...
def save_agent_as_template(
    agent_execution_id: str, organisation=Depends(get_user_organisation)
):
    """
    ...
    agent_executions = AgentExecution.get_agent_execution_from_id(
        db.session, agent_execution_id
    )
    ...
    agent_execution_configurations = (
        db.session.query(AgentExecutionConfiguration)
        .filter(AgentExecutionConfiguration.agent_execution_id == agent_execution_id)
        .all()
    )
    ...
    for agent_execution_configuration in agent_execution_configurations:
        ...
        if agent_execution_configuration.key == "tools":
            config_value = str(
                Tool.convert_tool_ids_to_names(
                    db, eval(agent_execution_configuration.value)
                )
            )
        agent_template_config = AgentTemplateConfig(
            agent_template_id=agent_template.id,
            key=agent_execution_configuration.key,
            value=config_value,
        )
...
def list_agent_templates(
    ...
    if template_source == "local":
        templates = (
            db.session.query(AgentTemplate)
            .filter(AgentTemplate.organisation_id == organisation.id)
            .all()
        )
        ...
        local_templates = (
            ...
            .filter(
                AgentTemplate.organisation_id == organisation.id,
                AgentTemplate.marketplace_template_id != None,
            )
            .all()
        )
...
def list_marketplace_templates(page=0):
    ...
    templates = (
        db.session.query(AgentTemplate)
        .filter(AgentTemplate.organisation_id == organisation_id)
...
def marketplace_template_detail(agent_template_id):
    ...
    template = (
        db.session.query(AgentTemplate)
        .filter(
            AgentTemplate.organisation_id == organisation_id,
            AgentTemplate.id == agent_template_id,
        )
        .first()
    )
    template_configs = (
        db.session.query(AgentTemplateConfig)
        .filter(AgentTemplateConfig.agent_template_id == template.id)
        .all()
    )

    workflow = (
        db.session.query(AgentWorkflow)
        .filter(AgentWorkflow.id == template.agent_workflow_id)
        .first()
    )
    tool_configs = {}
    for template_config in template_configs:
        config_value = AgentTemplate.eval_agent_config(
            template_config.key, template_config.value
        )
...
def download_template(
    agent_template_id: int, organisation=Depends(get_user_organisation)
):
    """
    ...
    template = AgentTemplate.clone_agent_template_from_marketplace(
        db, organisation.id, agent_template_id
    )
    return template.to_dict()


@router.get("/agent_config", status_code=201)
def fetch_agent_config_from_template(
    agent_template_id: int, organisation=Depends(get_user_organisation)
):
    """
    ...
    for config in template_config:
        if config.key in main_keys:
            template_config_dict[config.key] = AgentTemplate.eval_agent_config(
    ...
    agent_workflow = AgentWorkflow.find_by_id(
        db.session, agent_template.agent_workflow_id
    )

...
class ResourceSummarizer:
    ...
    def __get_model_api_key(self):
        return Configuration.fetch_configuration(
            self.session, self.organisation_id, "model_api_key"
        )

    def __get_model_source(self):
        return Configuration.fetch_configuration(
            self.session, self.organisation_id, "model_source"
        )
    ...
    def add_to_vector_store_and_create_summary(self, resource_id: int, documents: list):
        ...
        try:
            ResourceManager(str(self.agent_id)).save_document_to_vector_store(
                documents, str(resource_id), model_api_key, self.__get_model_source()
            )
        except Exception as e:
            logger.error(
                "add_to_vector_store_and_create_summary: Unable to save document to vector store.",
                e,
            )

    def generate_agent_summary(self, generate_all: bool = False) -> str:
        """Generate a summary of all resources for an agent."""
        agent_config_resource_summary = (
            self.session.query(AgentConfiguration)
            .filter(
                AgentConfiguration.agent_id == self.agent_id,
                AgentConfiguration.key == "resource_summary",
            )
            .first()
        )
        resources = (
            self.session.query(Resource)
            .filter(Resource.agent_id == self.agent_id, Resource.channel == "INPUT")
            .all()
        )
        if not resources:
            return
        ...
        agent_last_resource = (
            ...
            .filter(
                AgentConfiguration.agent_id == self.agent_id,
                AgentConfiguration.key == "last_resource_time",
            )
            .first()
        )
        ...
        if agent_config_resource_summary is not None:
            ...
            agent_config_resource_summary = AgentConfiguration(
                agent_id=self.agent_id, key="resource_summary", value=resource_summary
            )
        ...
        if agent_last_resource is not None:
            ...
            agent_last_resource = AgentConfiguration(
                agent_id=self.agent_id,
                key="last_resource_time",

...
class LlamaVectorStoreFactory:
    ...
    def get_vector_store(self) -> VectorStore:
        ...
        if self.vector_store_name == VectorStoreType.REDIS:
            redis_url = (
                get_config("REDIS_VECTOR_STORE_URL") or "redis://super__redis:6379"
            )
        ...
        if self.vector_store_name == VectorStoreType.QDRANT:
            ...
            return QdrantVectorStore(
                client=qdrant_client, collection_name=self.index_name
            )

        raise ValueError(
            str(self.vector_store_name) + " vector store is not supported yet."
        )

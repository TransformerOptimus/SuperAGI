...
class LlamaDocumentSummary:
    def __init__(
        self,
        model_name=get_config("RESOURCES_SUMMARY_MODEL_NAME", "gpt-3.5-turbo"),
        model_source="OpenAi",
    ...
    def generate_summary_of_document(self, documents: list[Document]):
        ...
        os.environ["OPENAI_API_KEY"] = (
            get_config("OPENAI_API_KEY", "") or self.model_api_key
        )
    ...
    def generate_summary_of_texts(self, texts: list[str]):
        ...
        if texts is not None and len(texts) > 0:
            documents = [
                Document(doc_id=f"doc_id_{i}", text=text)
    ...
    def _build_llm(self):
        ...
        if self.model_name in open_ai_models:
            ...
            return ChatOpenAI(
                temperature=0, model_name=self.model_name, openai_api_key=openai_api_key
            )

        raise Exception(
            f"Model name {self.model_name} not supported for document summary"
        )

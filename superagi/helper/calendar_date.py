...
class CalendarDate:
    def create_event_dates(self, service, start_date, start_time, end_date, end_time):
        ...
        start_datetime, end_datetime = self._localize_daterange(
            start_date, end_date, start_time, end_time, local_tz
        )
        date_utc = {
    ...
    def _localize_daterange(self, start_date, end_date, start_time, end_time, local_tz):
        start_datetime = (
            self._string_to_datetime(start_date, "%Y-%m-%d")
            if start_date != "None"
            else datetime.now(timezone.utc)
        )
        end_datetime = (
            self._string_to_datetime(end_date, "%Y-%m-%d")
            if end_date != "None"
            else start_datetime + timedelta(days=30) - timedelta(microseconds=1)
        )
        ...
        start_datetime = (
            start_datetime.replace(
                hour=time_obj_start.hour,
                minute=time_obj_start.minute,
                second=time_obj_start.second,
                microsecond=0,
            )
            if time_obj_start
            else start_datetime.replace(hour=0, minute=0, second=0, microsecond=0)
        )
        end_datetime = (
            end_datetime.replace(
                hour=time_obj_end.hour,
                minute=time_obj_end.minute,
                second=time_obj_end.second,
            )
            if time_obj_end
            else end_datetime.replace(hour=23, minute=59, second=59, microsecond=999999)
        )
        return self._convert_to_utc(start_datetime, local_tz), self._convert_to_utc(
            end_datetime, local_tz
        )

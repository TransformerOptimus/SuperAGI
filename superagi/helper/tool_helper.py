...
def download_tool(tool_url, target_folder):
    ...
    with zipfile.ZipFile(tool_zip_file_path, "r") as z:
        members = [
            m
            for m in z.namelist()
            if m.startswith(f"{owner}-{repo}") and f"{path}" in m
        ]
            ...
            for file_name in os.listdir(folder_dir):
                ...
                if file_name.endswith(".py") and not file_name.startswith("__init__"):
                    ...
                    update_base_tool_class_info(
                        classes, file_name, folder_name, session, tool_name_to_toolkit
                    )


def update_base_tool_class_info(
    classes, file_name, folder_name, session, tool_name_to_toolkit
):
    ...
    for folder_path in folder_paths:
        ...
        for folder_name in os.listdir(folder_path):
            ...
            for file_name in os.listdir(folder_dir):
                ...
                if file_name.endswith(".py") and not file_name.startswith("__init__"):
                    # Get classes
                    classes = get_classes_in_file(
                        file_path=file_path, clazz=BaseToolkit
                    )
                    tool_name_to_toolkit = update_base_toolkit_info(
                        classes,
                        code_link,
                        folder_name,
                        new_toolkits,
...
def delete_extra_toolkit(existing_toolkits, new_toolkits, session):
    for toolkit in existing_toolkits:
        if toolkit.name not in [new_toolkit.name for new_toolkit in new_toolkits]:
            ...
            session.query(ToolConfig).filter(
        ...
        if clazz["class_name"] is not None:
            ...
            for tool_config_key in tool_config_keys:
                if isinstance(tool_config_key, ToolConfiguration):
                    new_config = ToolConfig.add_or_update(
                        session,
                        toolkit_id=new_toolkit.id,
                    ...
                    ToolConfig.add_or_update(
                        session, toolkit_id=new_toolkit.id, key=tool_config_key
                    )
    return tool_name_to_toolkit
...
def process_files(folder_paths, session, organisation, code_link=None):
    existing_toolkits = (
        session.query(Toolkit).filter(Toolkit.organisation_id == organisation.id).all()
    )

    tool_name_to_toolkit = init_toolkits(
        code_link, existing_toolkits, folder_paths, organisation, session
    )
...
def get_readme_content_from_code_link(tool_code_link):
    ...
    readme_url = (
        f"https://raw.githubusercontent.com/{username}/{repository}/{branch}/README.MD"
    )
...
def register_toolkits(session, organisation):
    ...
    if organisation is not None:
        ...
        logger.info(
            f"Toolkits Registered Successfully for Organisation ID : {organisation.id}!"
        )
...
def register_marketplace_toolkits(session, organisation):
    ...
    if organisation is not None:
        process_files(tool_paths, session, organisation)
        logger.info(
            f"Marketplace Toolkits Registered Successfully for Organisation ID : {organisation.id}!"
        )
...
def compare_toolkit(toolkit1, toolkit2):
    ...
    toolkit_diff = any(
        toolkit1.get(field) != toolkit2.get(field) for field in main_toolkit_fields
    )
    ...
    if len(tools1) != len(tools2):
        ...
        tools_diff = any(
            compare_tools(tool1, tool2) for tool1, tool2 in zip(tools1, tools2)
        )

    tool_configs1 = sorted(
        toolkit1.get("configs", []), key=lambda config: config.get("key", "")
    )
    tool_configs2 = sorted(
        toolkit2.get("configs", []), key=lambda config: config.get("key", "")
    )
    if len(tool_configs1) != len(tool_configs2):
        tool_configs_diff = True
    else:
        tool_configs_diff = any(
            compare_configs(config1, config2)
            for config1, config2 in zip(tool_configs1, tool_configs2)
        )

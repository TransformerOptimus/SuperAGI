...
class S3Helper:
    ...
    def check_file_exists_in_s3(self, file_path):
        response = self.s3.list_objects_v2(
            Bucket=get_config("BUCKET_NAME"), Prefix="resources" + file_path
        )
    ...
    def get_download_url_of_resources(self, db_resources_arr):
        ...
        for db_resource in db_resources_arr:
            response = self.s3.get_object(
                Bucket=get_config("BUCKET_NAME"), Key=db_resource.path
            )
            ...
            object_key = (
                f"public_resources/run_id{db_resource.agent_execution_id}/{file_name}"
            )
    ...
    def list_files_from_s3(self, file_path):
        ...
        response = self.s3.list_objects_v2(
            Bucket=get_config("BUCKET_NAME"), Prefix=file_path
        )

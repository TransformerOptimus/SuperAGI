...
class GithubHelper:
    ...
    def check_repository_visibility(self, repository_owner, repository_name):
        ...
        if response.status_code == 200:
            ...
            logger.info(
                f"Failed to fetch repository information: {response.status_code} - {response.text}"
            )
            return None

    def search_repo(
        self, repository_owner, repository_name, file_name, folder_path=None
    ):
        """
    ...
    def sync_branch(
        self, repository_owner, repository_name, base_branch, head_branch, headers
    ):
        """
    ...
    def make_fork(self, repository_owner, repository_name, base_branch, headers):
        ...
        """
        fork_url = (
            f"https://api.github.com/repos/{repository_owner}/{repository_name}/forks"
        )
        ...
        if fork_response.status_code == 202:
            ...
            self.sync_branch(
                repository_owner, repository_name, base_branch, base_branch, headers
            )
        else:
    ...
    def create_branch(self, repository_name, base_branch, head_branch, headers):
        ...
        if branch_response.status_code == 201:
            ...
            logger.info(
                "Branch new-file already exists, making commits to new-file branch"
            )
        else:
    ...
    def delete_file(
        ...
        file_params = {
            ...
            "sha": self.get_sha(
                self.github_username, repository_name, file_name, folder_path
            ),
            "branch": head_branch,
        }
    ...
    def add_file(
        ...
        if file_response.status_code == 201:
            ...
            logger.info(
                "Failed to upload file content:", file_response.json()["message"]
            )
        return file_response.status_code

    def create_pull_request(
        self, repository_owner, repository_name, head_branch, base_branch, headers
    ):
        """
        ...
        pr_response = requests.post(
            pull_request_url, json=pull_request_params, headers=headers
        )

...
class GoogleCalendarCreds:
    ...
    def get_credentials(self, toolkit_id):
        ...
        google_creds = (
            self.session.query(OauthTokens)
            .filter(
                OauthTokens.toolkit_id == toolkit_id,
                OauthTokens.organisation_id == organisation_id,
            )
            .first()
        )
        if google_creds:
            ...
            final_creds["refresh_token"] = self.fix_refresh_token(
                final_creds["refresh_token"]
            )
            expire_time = datetime.strptime(
                final_creds["expiry"], "%Y-%m-%dT%H:%M:%S.%fZ"
            )
            google_creds = (
                self.session.query(ToolConfig)
                .filter(ToolConfig.toolkit_id == toolkit_id)
                .all()
            )
            client_id = ""
            client_secret = ""

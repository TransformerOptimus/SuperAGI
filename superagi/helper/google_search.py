...
class GoogleSearchAPIWrap(BaseModel):

    """
    A wrapper class for google search functionality in SuperAGI.
    """
    ...
    class GoogleSearch:
        ...
        def __init__(self, api_key, search_engine_id, num_results=10, num_pages=1):
        ...
        def search_google(self, query, start_index):
            results, error_code = self.search(query, start_index)
        ...
        def get_snippets(self, query):
            ...
            for page in range(1, self.num_pages * self.num_results, self.num_results):
                snippets, link, error_code = self.search_google(query, start_index=page)
    ...
    class WebpageExtractor:
        ...
        def get_toplist(self, query, google_search):
            snippets, links, error_code = google_search.get_snippets(query)
            
            # if error_code == 429:
            #     snippets, links = GoogleSearch.search_google(query)
            ...
            if links:
                for i in range(0, self.num_extracts):
                    time.sleep(3)
                    content = self.extract_with_3k(links[i])
                    attempts = 0
                    ...
                    if content == "":
                        time.sleep(3)
                        content = self.extract_with_bs4(links[i])
                        attempts = 0
                        while content == "" and attempts < 2:
                            attempts += 1
                            content = self.extract_with_bs4(links[i])
...
# Example usage:
# api_key = "AIzaSyCinJK2sB7Ky-3DmmsSNWFn5leAwev-9Hs"
# search_engine_id = "e6fbba732ab9e49fd"
# query = "Give me data of all the latest basketball matches"

# google_search = GoogleSearch(api_key, search_engine_id)
# webpage_extractor = WebpageExtractor()

# snippets, webpages, links = webpage_extractor.get_toplist(query, google_search)
